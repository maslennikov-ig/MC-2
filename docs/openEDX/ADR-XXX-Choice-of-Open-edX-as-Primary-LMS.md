...
- **RU-Stack (Yandex Cloud - Moscow):**
  - _DNS:_ `*.megacampus.ru`
  - _Auth:_ Keycloak (Local) or Russian OAuth.
  - _Logs:_ Retained for 1 year (Yarovaya Law) in Cold Storage.
  - _Payment:_ Mir/SberPay integration.
- **Intl-Stack (AWS - Frankfurt/Dubai):**
  - _DNS:_ `*.megacampus.io`
  - _Auth:_ Auth0 or AWS Cognito.
  - _Compliance:_ GDPR Delete requests, Cookie consent (OneTrust).
  - _Payment:_ Stripe integration.
