...
### Что надо доказать до правки, а не после — ДОКАЗАНО 2026-08-21, ответ отрицательный

Вопрос был: проходит ли `withStructuredOutput(...).invoke()` через подменённый
`model.invoke`. Ответ — **нет**, и предположение «колбэки этот путь переживают» тоже
оказалось неверным.

`ChatOpenAI.withConfig` (через него идут и `withStructuredOutput`, и `bindTools`)
реализован как `new ChatOpenAI(this.fields)`. Клон получает поля конструктора и больше
ничего, поэтому теряется и подменённый `invoke`, и `model.callbacks`, которые ставил
`attachCostRecording`. Это не баг: до 0.6.3 `bindTools` писал в общий экземпляр, и два
агента с разными инструментами видели чужие (langchainjs#8586) — клонирование и есть
починка. В 1.4.7 и 1.5.10 поведение одинаковое, обновление ничего не меняет.

Следствие, которого план не знал: **структурированные вызовы не записывали стоимость
вообще**. Тест-доказательство:
...
**Сделано не так, как здесь было написано.** Обёртка `invoke` и карман не годятся: см.
раздел выше. Решение владельца 2026-08-21 — идти публичным API:

- Колбэки записи стоимости передаются **в конструктор** модели, а не навешиваются на
  готовую. Конструкторные колбэки лежат в `this.fields`, поэтому клон их сохраняет —
  это документированная семантика, а не обход версии. `attachCostRecording` заменён на
  `costRecordingCallbacks` + `createCostRecordingModel`/`Async` в фабрике; восемь мест
  вызова переписаны, `getModelForPhase` передаёт контекст в фабрику.
- Номер вызова берётся из ответа: `generations[0][0].message.id` — это тот же `gen-…`,
  что и в заголовке `x-generation-id`, и именно его принимает `/api/v1/generation?id=`.
  Карман на курсовом пути не нужен вовсе.
- Инструментированный `fetch` остаётся: при обрыве `handleLLMEnd` не срабатывает, и там
  карман — единственный источник. На нём держится career playbook.
- `withMandatoryReasoningRecovery` больше не переносит `model.callbacks` руками: пересборка
  идёт через ту же фабрику и получает их из конструктора. Но сам он по-прежнему подменяет
  `invoke` на экземпляре и потому **не покрывает четыре структурированных места** — это
  отдельный дефект `mc2-148j9`, не закрытый здесь.
...
### F. Каталог: пять снятых записей — `mc2-g1zt9`

**Поправка 2026-08-21:** записей оказалось пять, а не четыре. Пятая —
`deepseek/deepseek-v4-flash` 0.0826/0.1652 против живых 0.0798/0.1596. Она НЕ безобидна
как остальные: `normalizeModelId` считает по ней каждый недатированный снимок V4 Flash,
то есть она на живом маршруте. Расхождение в сторону завышения, поэтому потолок
`max_price` из неё вызовы не ронял.

Решение по CI: `--all` в блокирующий CI **не добавляем**. Цена — факт о чужом сервисе, и
смена тарифа провайдером не должна ронять нашу сборку и вместе с ней деплой. Расписанная
неблокирующая проверка заведена как `mc2-ts9i2`.
