...
> **Проверено 2026-08-21 — абзац выше неверен.** Первый вызов specBuilder
> (`spec-builder.ts:367`) не передаёт `preferFallbackModel`; строка 407 лежит внутри
> `catch (retryError)`, то есть это эскалация после того, как primary уже дважды не
> справился. luna пробуется первой. Настоящий механизм — `attempt > 0`: при
> `max_retries = 3` попытки 1–3 уходили на медленный алиас против таймаута фазы
> 120 000 мс, и четыре таймаута подряд были одним вызовом из четырёх попыток.
> Подробности и решение владельца — `mc2-rqukn`.
...
> **Выполнено 2026-08-21 не так, как написано ниже.** Проверка показала, что все три
> флага — это эскалации после провала, а не постоянный пин, и luna уже пробуется первой.
> Решение владельца 2026-08-21: флаги не трогать, починить `attempt > 0`. Сделано:
> `FALLBACK_FROM_ATTEMPT = 2` в `nodes/runtime.ts` — попытка 1 повторяет primary с
> исключением подведшего провайдера (раздел B), на fallback уходят попытки 2+. Разбор в
> `mc2-rqukn`.
