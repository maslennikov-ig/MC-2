...
Чего измерение не отвечает: **качество**. Ставка ниже, вызов дешевле, но пишет
ли модель текст, который стоит читать, решается прогоном
`career-playbook-model-ab.ts` (метка `glm-flash` добавлена) и чтением
артефактов, а не оценкой судьи.
