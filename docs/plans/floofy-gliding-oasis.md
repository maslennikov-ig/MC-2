...
2. **`:batch` не даёт ничего.** Batch по default-тарифу стоит ровно столько же,
   сколько синхронный flex, а batch@flex через Batch API недостижим — проверено
   двумя оплаченными вызовами (см. Этап 3). Включённый сегодня batch дал бы ноль
   экономии и добавил бы окно до 24 часов.
