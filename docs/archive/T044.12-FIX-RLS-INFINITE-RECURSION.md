...
**Target**: +8 pgTAP tests → 176/181 passing (97%)
...
- ❌ **8 pgTAP RLS tests cannot run** (all fail immediately)
- ⚠️ **Production database may have same issue** (RLS policies are live)
- 🔒 **Security risk**: If RLS fails, it may default to allowing access or denying all access

**Root Cause**: Circular dependency in RLS policies:

```
users table RLS → checks courses table
courses table RLS → checks users table
...
USING (
  organization_id IN (
    ...
    FROM users
    WHERE id = auth.uid()  -- ← RECURSION: queries same table
  )
);
```
...
USING (
  organization_id IN (
    SELECT organization_id
    FROM users
    WHERE id = auth.uid()  -- ← Queries users table
  )
);
```
...
1. Query `users` table → triggers RLS policy
2. RLS policy queries `users` table again → triggers RLS policy again
...
### Option 1: Use JWT Claims (RECOMMENDED ⭐)
...
- ✅ No recursion (doesn't query users table)
- ✅ Fast (reads from JWT, no DB query)
- ✅ Secure (JWT signed by Supabase)
- ✅ Scalable (no additional DB load)

**Disadvantages**:

- ⚠️ Requires re-login after organization change
- ⚠️ JWT must be refreshed if organization_id changes
...
- ✅ No recursion (function bypasses RLS)
- ✅ Works immediately (no JWT refresh needed)
- ✅ Dynamic (picks up organization changes instantly)

**Disadvantages**:

- ⚠️ Additional DB query for each RLS check
- ⚠️ Security risk if function not carefully written
- ⚠️ Performance impact (queries DB on every access)
...
- ✅ No recursion (context table has simple RLS)
- ✅ Can cache in application layer
- ✅ Flexible (can add more context fields)

**Disadvantages**:

- ⚠️ Extra table to maintain
- ⚠️ Sync issues if users/organizations change
...
- [ ] All 8 tests passing (168→176 total passing tests)
...
## 📝 Analysis Summary (2025-10-12)

**Analysis Performed By**: Claude Code with Docs L1/L2 documentation review
...
- Creates infinite recursion: `users RLS → SELECT FROM users → users RLS → ...`
- Blocking 8 pgTAP tests from running

📚 **Documentation Review** (Supabase Official Docs):
...
**Confidence Level**: 🟢 **VERY HIGH (99%)** - Standard JWT claims pattern, infrastructure ready
