...
_Immediate attention required - Data corruption risks, race conditions_
...
await supabase.from('llm_model_config').update({ is_active: false }).eq('id', input.id);
...
await supabase.from('prompt_templates').update({ is_active: false }).eq('id', currentActive.id);
...
_Should be fixed before deployment - Security, data integrity_
...
_Should be scheduled for fixing - Type safety, error handling_
...
_No changes made - read-only analysis_
