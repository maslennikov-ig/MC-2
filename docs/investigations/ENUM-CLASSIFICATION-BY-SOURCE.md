...
**Validation**: ✅ **STRICT** - These are business/architectural constraints
...
**Total**: 11 fields
**Recommendation**: ✅ **Keep strict validation** - these are NOT generated by LLM, they are set by code logic
...
**Validation**: ✅ **STRICT** - User can only select from predefined options
...
**Recommendation**: ✅ **Keep strict validation** - these come from user UI, not LLM
...
**Validation**: ✅ **STRICT** - These are set by code, not LLM
...
**Total**: 3 fields
**Recommendation**: ✅ **Keep strict validation** - these are code-generated, not LLM-generated
...
**Validation**: ✅ **STRICT** - These are injected by code with known-good values
...
**Recommendation**: ✅ **Keep strict validation** - these are NOT generated by LLM, injected by code
...
**Validation**: ⚠️ **FLEXIBLE** - These are LLM-to-LLM communication, NOT database fields

#### 2.1.1 Category/Classification Fields (Phase 1)

| Field                       | Schema                | Values                                                                   | Stage 4 → Stage 5           | Current Issue                               |
| --------------------------- | --------------------- | ------------------------------------------------------------------------ | --------------------------- | ------------------------------------------- |
| `course_category.primary`   | `analysis-schemas.ts` | `professional`, `personal`, `creative`, `hobby`, `spiritual`, `academic` | Advisory for tone/style     | ⚠️ Validation failures if LLM uses synonyms |
| `course_category.secondary` | `analysis-schemas.ts` | Same as primary                                                          | Optional secondary category | ⚠️ Validation failures                      |
| `complexity`                | `analysis-schemas.ts` | `narrow`, `medium`, `broad`                                              | Topic complexity hint       | ⚠️ Validation failures                      |
| `target_audience`           | `analysis-schemas.ts` | `beginner`, `intermediate`, `advanced`, `mixed`                          | Difficulty recommendation   | ⚠️ Validation failures                      |

#### 2.1.2 Pedagogical Guidance Fields (Phase 1)

| Field              | Schema                | Values                                                                                       | Stage 4 → Stage 5          | Current Issue            |
| ------------------ | --------------------- | -------------------------------------------------------------------------------------------- | -------------------------- | ------------------------ |
| `primary_strategy` | `analysis-schemas.ts` | `problem-based learning`, `lecture-based`, `inquiry-based`, `project-based`, `mixed`         | Teaching approach          | ⚠️ Validation failures   |
| `assessment_types` | `analysis-schemas.ts` | `coding`, `quizzes`, `projects`, `essays`, `presentations`, `peer-review`                    | Assessment recommendations | ⚠️ Validation failures   |
| `tone`             | `generation-guidance` | `conversational but precise`, `formal academic`, `casual friendly`, `technical professional` | Writing tone guidance      | ⚠️ Validation failures   |
| `include_visuals`  | `generation-guidance` | `diagrams`, `flowcharts`, `code examples`, `screenshots`, `animations`, `plots`              | Visual content suggestions | ⚠️ Validation failures   |
| `exercise_types`   | `generation-guidance` | `coding`, `derivation`, `interpretation`, `debugging`, `refactoring`, `analysis`             | **Exercise type guidance** | **🔴 T053 FAILURE HERE** |

#### 2.1.3 Internal Stage 4 Fields (Not Used by Stage 5)

| Field                         | Schema                | Values                                 | Used By                   | Recommendation                     |
| ----------------------------- | --------------------- | -------------------------------------- | ------------------------- | ---------------------------------- |
| `importance`                  | `analysis-schemas.ts` | `core`, `important`, `optional`        | Stage 4 internal priority | ℹ️ No validation needed (semantic) |
| `difficulty_progression`      | `analysis-schemas.ts` | `flat`, `gradual`, `steep`             | Stage 4 internal          | ℹ️ No validation needed (semantic) |
| `difficulty`                  | `analysis-schemas.ts` | `beginner`, `intermediate`, `advanced` | Section-level difficulty  | ℹ️ No validation needed (semantic) |
| `document_processing_methods` | `document-relevance`  | `full_text`, `hierarchical`            | RAG processing hint       | ℹ️ No validation needed (internal) |

**Total Stage 4 fields**: 13 advisory + 4 internal = **17 fields**
**Recommendation**: ⚠️ **Apply FLEXIBLE validation** (warnings only) - these are LLM-to-LLM guidance
...
4. **The validation failure is happening at Stage 4 → Stage 5 handoff, NOT at Stage 5 → Database**
...
**Validation**: ✅ **STRICT** - These fields go to database and must match schema

#### 2.2.1 Database-Bound Fields (MUST Match Database Schema)

| Field              | Schema                 | Values                                                                                        | Database Column                                                                               | Validation Strategy                       |
| ------------------ | ---------------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | ----------------------------------------- |
| `difficulty_level` | `generation-result.ts` | `beginner`, `intermediate`, `advanced`                                                        | `generation_result.course_structure.difficulty_level`                                         | ✅ **STRICT** - Database field            |
| `exercise_type`    | `generation-result.ts` | `self_assessment`, `case_study`, `hands_on`, `discussion`, `quiz`, `simulation`, `reflection` | `generation_result.course_structure.sections[].lessons[].practical_exercises[].exercise_type` | ✅ **STRICT** - Database field            |
| `cognitiveLevel`   | `generation-result.ts` | `remember`, `understand`, `apply`, `analyze`, `evaluate`, `create`                            | `generation_result.course_structure.learning_outcomes[].cognitiveLevel`                       | ✅ **STRICT** - Database field (optional) |

**Total**: 3 fields (`exercise_type` has 7 values, `difficulty_level` has 3, `cognitiveLevel` has 6, `cognitiveLevel` is optional)
**Recommendation**: ✅ **Keep strict validation** - these are stored in database JSONB
...
| -------------------------------- | ------ | -------------------- | --------------------------------------------- |
| **Database constraints**         | 11     | ✅ **Strict**        | Business logic requires exact values          |
| **Frontend input**               | 2      | ✅ **Strict**        | User selections from predefined UI            |
| **Algorithmic logic**            | 3      | ✅ **Strict**        | Code-generated, deterministic                 |
| **Code-injected (post-LLM)**     | 2      | ✅ **Strict**        | Injected by code with known-good values       |
| **Stage 4 LLM (advisory)**       | 13     | ⚠️ **Flexible**      | Recommendations for Stage 5, NOT stored in DB |
| **Stage 4 LLM (internal)**       | 4      | ℹ️ **No validation** | Internal Stage 4 metadata                     |
| **Stage 5 LLM (database-bound)** | 3      | ✅ **Strict**        | Must match database schema                    |
| **TOTAL**                        | **38** | Mixed                | -                                             |
...
- Stage 4 → Stage 5 handoff should use flexible/semantic validation
- Only Stage 5 → Database should use strict validation
...
exercise_types: z.array(
  z.string().refine(val => {
    ...
    if (!validTypes.includes(val)) {
      console.warn(
        `[Stage 4] Non-standard exercise type: "${val}" (expected: ${validTypes.join(', ')})`
      );
    }
...
1. `packages/shared-types/src/analysis-schemas.ts` (13 fields)
   ...
   - `tone`
   - `include_visuals`
   - `exercise_types` (🔴 **THIS IS CAUSING T053 FAILURE**)

**Expected impact**:

- ✅ T053 test passes (Stage 4 no longer blocks on `exercise_types: ['analysis']`)
- ✅ Stage 5 receives advisory guidance
- ✅ Stage 5 interprets semantically and outputs valid database values
- ✅ Quality remains high (Stage 5 → Database validation still strict)
