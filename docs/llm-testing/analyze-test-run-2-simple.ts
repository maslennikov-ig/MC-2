...
interface ModelAnalysis {
  model: string;
  metadataEN: CategoryScore;
  metadataRU: CategoryScore;
  lessonEN: CategoryScore;
  lessonRU: CategoryScore;
  overallScore: number;
}
...
function assessMetadataQuality(data: MetadataResult, language: 'en' | 'ru'): QualityScore {
...
function assessLessonQuality(data: LessonResult, language: 'en' | 'ru'): QualityScore {
  const details: string[] = [];
  let schemaPoints = 0;
  let contentPoints = 0;
  let languagePoints = 0;
...
function analyzeModel(modelSlug: string): ModelAnalysis {
...
function generateBilingualReport(results: ModelAnalysis[]) {
...
const allResults: ModelAnalysis[] = [];
...
writeFileSync(outputJSON, JSON.stringify(allResults, null, 2), 'utf-8');
