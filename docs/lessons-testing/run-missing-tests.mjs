/**
 * Run ONLY missing tests in parallel
 *
 * Missing tests:
 * - qwen3-235b-thinking: 1.1-en, 1.2-en
 * - devstral-2512: all 4 tests (1.1, 1.2, 1.1-en, 1.2-en)
 */
...
// Missing tests to run
const MISSING_TESTS = [
  { model: 'qwen3-235b-thinking', lesson: '1.1-en' },
  { model: 'qwen3-235b-thinking', lesson: '1.2-en' },
  { model: 'devstral-2512', lesson: '1.1' },
  { model: 'devstral-2512', lesson: '1.2' },
  { model: 'devstral-2512', lesson: '1.1-en' },
  { model: 'devstral-2512', lesson: '1.2-en' },
];
...
async function runSingleTest(modelSlug, lessonId) {
  const model = MODELS[modelSlug];
  const lesson = LESSONS[lessonId];
  const startTime = Date.now();
  const testId = `${modelSlug}_${lessonId}`;

  try {
    console.log(`  [${modelSlug}] ${lessonId} (${lesson.language}) - Starting...`);
    ...
    const status = parsed
      ...
      : 'JSON parse error';
    console.log(`  [${modelSlug}] ${lessonId} - ${status} (${(duration / 1000).toFixed(1)}s)`);
    ...
    console.log(`  [${modelSlug}] ${lessonId} - Error: ${errorMsg.substring(0, 80)}`);
...
async function main() {
  ...
  console.log('');
  console.log('        RUNNING MISSING TESTS IN PARALLEL');
  console.log('');
  console.log('='.repeat(70));
  console.log('');

  console.log('Missing tests to run:');
  MISSING_TESTS.forEach(t => console.log(`  - ${t.model} / ${t.lesson}`));
  console.log(`\nTotal: ${MISSING_TESTS.length} tests`);
  console.log('');

  const startTime = Date.now();

  // Run ALL missing tests in PARALLEL
  console.log('Running all tests in PARALLEL...\n');
  const results = await Promise.all(
    MISSING_TESTS.map(t => runSingleTest(t.model, t.lesson))
  );
  ...
  results.forEach(r => {
    const status = r.parseSuccess ? `Score: ${r.quality?.qualityScore || 0}/100` : `Error: ${r.error?.substring(0, 50) || 'parse error'}`;
    console.log(`  [${r.modelSlug}] ${r.lessonId} - ${status}`);
  });

  console.log('');
  console.log('Done! Check test-results/ for detailed output.');
}
