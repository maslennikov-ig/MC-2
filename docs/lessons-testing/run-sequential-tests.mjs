 ...
 * Models: DeepSeek V3.2, Devstral 2512, Nemotron 3 Nano 30B (paid)
 */
...
const MODELS = [
  {
    slug: 'deepseek-v3.2-seq',
    apiName: 'deepseek/deepseek-v3.2',
    name: 'DeepSeek V3.2 (Sequential)'
  },
  {
    slug: 'devstral-2512-seq',
    apiName: 'mistralai/devstral-2512:free',
    name: 'Devstral 2512 (Sequential)'
  },
  {
    slug: 'nemotron-nano-30b-paid',
    apiName: 'nvidia/nemotron-3-nano-30b-a3b',
    name: 'Nemotron 3 Nano 30B (Paid)'
  }
];
...
CRITICAL: Return ONLY valid JSON. No markdown code blocks around the JSON, no explanations before or after. Use only standard ASCII quotes (\\") in JSON strings.`;
}
...
async function runSingleTest(model, lessonId) {
...
async function runModelTestsSequentially(model) {
  console.log(`\n${'='.repeat(60)}`);
  ...
  // Run tests SEQUENTIALLY with delay between each
  for (const lessonId of ALL_LESSONS) {
    const result = await runSingleTest(model, lessonId);
    results.push(result);
  ...
  console.log(`\n  Summary: ${successful.length}/4 success, avg score: ${avgScore.toFixed(0)}/100, time: ${(duration / 1000).toFixed(0)}s`);

  return { model, results, successful: successful.length, avgScore, duration };
}

async function main() {
  console.log('='.repeat(70));
  console.log('');
  console.log('     SEQUENTIAL TESTS (DeepSeek, Devstral, Nemotron Paid)');
  console.log('');
  console.log('='.repeat(70));
  ...
  for (const model of MODELS) {
    const modelResult = await runModelTestsSequentially(model);
    allModelResults.push(modelResult);
    // Wait 5 seconds between models
    await sleep(5000);
  }
  ...
  for (const { model, successful, avgScore, duration } of allModelResults) {
    console.log(`  ${model.name.padEnd(35)} ${successful}/4  Score: ${avgScore.toFixed(0).padStart(3)}/100  (${(duration / 1000).toFixed(0)}s)`);
  }
  console.log('');
}
