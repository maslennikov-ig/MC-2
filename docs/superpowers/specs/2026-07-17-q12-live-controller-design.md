...
All citations below are derived from bytes in this worktree (`codex/q12-live-controller`, HEAD
`14b60142d`); frozen trio sha256 unchanged (`aaec6fc2…` manifest, `3673ee49…` barrier, `0b8a943f…`
catalog). Where the runtime fixture
...
**Doc-forward supersession (§5.5 and the R5-D2 error text).** §5.5's "Composed recover procedure
(RULING 2 option (a))" clause (`:397-429`) — which fixes the supported set to "exactly three clean
checkpoints" plus the witness-file activate-head dispatch — is **superseded** by this table. The
mid-barrier two-step composition (§5.5 `:416-425`) is retained **as-is** but is now the fail-closed
branch of the one table (not a separate procedure), and its R8 proof obligation (§5.5 `:426-429`)
carries forward unchanged. The R5-D2 error-pointer text (`q12-lifecycle-core.py:3755-3758`) is
correct as written and remains the operator hand-off for mid-barrier heads.
