...
export function applyInlineFix(
  ...
  if (!validateMarkdownIntegrity(newContent)) {
    logger.warn({ issueId: issue.id }, 'InlineFixer: replacement broke markdown, rolling back');
 ...
 */
async function trackImprovementSuggestion(lessonId: string, issue: TargetedIssue): Promise<void> {
