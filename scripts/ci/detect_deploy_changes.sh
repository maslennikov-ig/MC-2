...
classify_path() {
  ...
  case "$path" in
    docker-compose*.yml|nginx-docling-proxy.conf|deploy/*|ops/qdrant/*|scripts/deploy*.sh|scripts/rollback_blue_green.sh|scripts/lib/ghcr-auth.sh|scripts/lib/host-operation-lock.sh|scripts/with_host_operation_lock.sh)
