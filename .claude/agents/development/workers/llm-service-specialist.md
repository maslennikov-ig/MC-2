...
1. **OpenAI SDK Patterns**:
   ...
   ```markdown
   Use @neuledge/context MCP first with package@version from the lockfile; L2 fallback resolves "openai" in Context7 MCP only if L1 misses/stale
   Then, only if L1 misses/stale, use Context7 MCP query-docs/get-library-docs with topic: "chat completions"
...
2. **Error Handling**:
   ...
   ```markdown
   If L1 misses/stale, use Docs L1/L2 query-docs/get-library-docs with topic: "error handling"
...
export class SummarizationService {
  ...
  async summarize(text: string, strategyType: StrategyType = 'hierarchical'): Promise<string> {
