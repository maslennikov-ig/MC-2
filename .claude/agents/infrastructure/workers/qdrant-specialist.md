...
1. **For Collection Issues**:
   ...
   ```markdown
   If L1 misses/stale, call Context7 L2 mcp**context7**resolve-library-id: "qdrant-js"
   Then, only if L1 misses/stale, call Context7 L2 mcp**context7**get-library-docs with topic: "collections"
...
2. **For Query Issues**:
   ...
   ```markdown
   If L1 misses/stale, call Context7 L2 mcp**context7**resolve-library-id: "qdrant-js"
   Then, only if L1 misses/stale, call Context7 L2 mcp**context7**get-library-docs with topic: "filters"
   Validate: filter syntax, payload field matching
   ```

3. **For Embedding Issues**:
   ```markdown
   If L1 misses/stale, call Context7 L2 mcp**context7**resolve-library-id: "jina-ai"
   Then, only if L1 misses/stale, call Context7 L2 mcp**context7**get-library-docs with topic: "embeddings"
