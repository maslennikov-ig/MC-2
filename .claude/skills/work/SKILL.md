...
**Before implementing any task with external dependencies, query @neuledge/context first using the lockfile-routed package/version; use Docs L1/L2 MCP only when L1 is missing, stale, or insufficient.**
...
```
1. @neuledge/context MCP — query local package@version from lockfile
2. Context7 MCP — fallback only when L1 is missing, stale, or insufficient
```
...
2. **Use Docs L1/L2** for documentation on involved libraries (MANDATORY)
...
2. Docs L1/L2: @neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient (MANDATORY)
...
# Step 1: @neuledge/context MCP with package@version from lockfile
# Step 2: Context7 MCP fallback only for L1 miss/stale/insufficient
```
...
- [ ] Docs L1/L2 checked for involved libraries (mandatory)
