#!/usr/bin/env bash
#
# Dev Delivery Script - Merge current working branch into develop and push develop
#
# Usage: ./push-dev.sh [--yes] [-y]
#        --yes, -y: Skip confirmation prompt
#
# Notes:
# - This is a Dev delivery path only.
# - It does not create release tags or version bumps.
# - It never touches main/master or staging deploy flows.

set -euo pipefail
trap '' SIGPIPE

readonly SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
readonly PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"

readonly RED='\033[0;31m'
readonly GREEN='\033[0;32m'
readonly YELLOW='\033[1;33m'
readonly BLUE='\033[0;34m'
readonly NC='\033[0m'

stash_needed="false"

log_info() { echo -e "${BLUE}[info] $*${NC}"; }
log_success() { echo -e "${GREEN}[ok] $*${NC}"; }
log_warning() { echo -e "${YELLOW}[warn] $*${NC}"; }
log_error() { echo -e "${RED}[error] $*${NC}" >&2; }

sync_beads() {
    if ! command -v bd >/dev/null 2>&1; then
        return 0
    fi

    log_info "Syncing Beads..."

    if bd dolt push --help >/dev/null 2>&1; then
        if bd dolt push; then
            log_success "Beads Dolt remote pushed"
        else
            log_warning "Beads Dolt push failed; continuing with git delivery"
        fi
        return 0
    fi

    log_warning "Installed bd has no supported sync command; continuing with git delivery"
}

cleanup() {
    local exit_code=$?
    trap - EXIT

    if [ "$stash_needed" = "true" ]; then
        local top_stash
        top_stash="$(git stash list --format='%gd' | sed -n '1p')"
        if [ -n "$top_stash" ]; then
            log_info "Restoring stashed local changes..."
            if ! git stash pop --quiet; then
                log_warning "Could not restore stashed changes automatically"
                log_warning "Run 'git stash list' and 'git stash pop' manually if needed"
                exit_code=1
            fi
        fi
    fi

    exit "$exit_code"
}

show_usage() {
    cat <<'EOF'
Usage: bash .claude/scripts/push-dev.sh [--yes|-y]

Promote the current working branch into develop and push develop.
This is a Dev delivery command, not a release command.
EOF
}

# Delivery is the moment to notice work that never got delivered. The 2026-07-27 branch audit
# found three finished, reviewed, bead-closed changes stranded on side branches for weeks
# (mc2-jc275, mc2-v31gc, mc2-sjpbx), because bead status records intent and not delivery.
# Advisory only: develop is already pushed by the time this runs, so it must never fail here.
# Delete the branch we just delivered, so it stops being a branch.
#
# This is where 200 remote and 69 local branches came from by 2026-08-22: every delivery left
# its source branch behind, for ever, and the only thing that ever noticed was the
# stranded-commit report — which by design cannot tell "finished and delivered" from
# "abandoned". A delivered branch is not history; the commits are in develop and stay there.
#
# Refuses to act unless git itself confirms the branch is contained in develop. That is the
# whole safety argument: if `--is-ancestor` says yes, nothing can be lost by removing the name.
cleanup_delivered_branch() {
    local branch="$1"

    case "$branch" in
        develop | master | main) return 0 ;;
        archive/* | backup/*) return 0 ;;
    esac

    # Anything the repository has knowingly parked stays parked.
    if grep -qE "^${branch}[[:space:]]" "$PROJECT_ROOT/.codex/stranded-commit-allowlist.txt" 2>/dev/null; then
        log_info "Keeping $branch: it is in the stranded-commit allowlist"
        return 0
    fi

    if ! git merge-base --is-ancestor "$branch" develop 2>/dev/null; then
        log_warning "Keeping $branch: develop does not contain it yet"
        return 0
    fi

    if [ -n "$(git status --porcelain 2>/dev/null)" ]; then
        log_warning "Keeping $branch: the worktree has uncommitted changes"
        return 0
    fi

    git checkout develop --quiet 2>/dev/null || return 0
    if git branch -d "$branch" >/dev/null 2>&1; then
        log_success "Deleted delivered branch $branch"
    fi
    if git ls-remote --exit-code --heads origin "$branch" >/dev/null 2>&1; then
        if git push origin --delete "$branch" >/dev/null 2>&1; then
            log_success "Deleted origin/$branch"
        fi
    fi
}

report_stranded_commits() {
    local checker="$PROJECT_ROOT/scripts/orchestration/check_stranded_commits.py"
    if [ ! -f "$checker" ] || ! command -v python3 >/dev/null 2>&1; then
        return 0
    fi

    local output
    if output="$(python3 "$checker" --repo "$PROJECT_ROOT" 2>&1)"; then
        log_info "Stranded-commit check: clean"
        return 0
    fi

    log_warning "Undelivered work found on other branches:"
    echo "$output"
    log_warning "Deliver it, or record the branch in .codex/stranded-commit-allowlist.txt with a reason"
    return 0
}

main() {
    cd "$PROJECT_ROOT"

    echo ""
    echo "============================================================="
    echo "Dev Delivery"
    echo "============================================================="
    echo ""

    local auto_confirm="false"

    while [[ $# -gt 0 ]]; do
        case "$1" in
            --yes|-y) auto_confirm="true"; shift ;;
            --help|-h)
                show_usage
                return 0
                ;;
            *)
                log_error "Unknown argument: $1"
                show_usage
                return 1
                ;;
        esac
    done

    local current_branch
    current_branch="$(git branch --show-current)"

    if [ -z "$current_branch" ]; then
        log_error "Detached HEAD is not supported"
        return 1
    fi

    log_info "Current branch: $current_branch"

    case "$current_branch" in
        main|master)
            log_error "Refusing to run from protected branch '$current_branch'"
            echo "Checkout a feature branch or develop first:"
            echo "  git checkout feature/<name>"
            echo "  git checkout codex/<name>"
            echo "  git checkout develop"
            return 1
            ;;
        develop)
            ;;
        *)
            log_info "Using non-protected source branch: $current_branch"
            ;;
    esac

    if ! git rev-parse --verify develop >/dev/null 2>&1; then
        log_error "Branch 'develop' does not exist"
        return 1
    fi

    stash_needed="false"
    if [ -n "$(git status --porcelain)" ]; then
        log_warning "Uncommitted changes detected; stashing them temporarily"
        git stash push --include-untracked -m "push-dev-temp-stash" >/dev/null
        stash_needed="true"
    fi

    trap cleanup EXIT

    local original_branch="$current_branch"

    if [ "$current_branch" != "develop" ]; then
        log_info "Switching to develop..."
        git checkout develop --quiet
    fi

    log_info "Fetching latest develop from origin..."
    git fetch origin develop --quiet 2>/dev/null || true

    log_info "Fast-forwarding develop from origin..."
    if ! git pull --ff-only origin develop; then
        log_error "Unable to fast-forward develop from origin"
        echo "Resolve develop sync first, then rerun this command."
        return 1
    fi

    local commits_ahead="0"
    if [ "$original_branch" = "develop" ]; then
        commits_ahead="$(git rev-list --count origin/develop..develop 2>/dev/null || echo "0")"
    else
        commits_ahead="$(git rev-list --count develop.."$original_branch" 2>/dev/null || echo "0")"
    fi

    if [ "$commits_ahead" -eq 0 ] && [ "$original_branch" != "develop" ]; then
        log_warning "No new commits to promote from $original_branch into develop"
        echo "$original_branch is already merged into develop"
        return 0
    fi

    if [ "$original_branch" = "develop" ] && [ "$commits_ahead" -eq 0 ]; then
        log_warning "develop is already up to date with origin/develop"
        return 0
    fi

    if [ "$original_branch" != "develop" ]; then
        log_success "Found $commits_ahead commit(s) to promote from $original_branch"
        echo ""
        log_info "Commits to promote ($original_branch -> develop):"
        git log develop.."$original_branch" --oneline -20
        if [ "$commits_ahead" -gt 20 ]; then
            echo "  ... and $((commits_ahead - 20)) more commits"
        fi
        echo ""

        if [ "$auto_confirm" != "true" ]; then
            if [ -t 0 ]; then
                read -p "Promote these $commits_ahead commit(s) to develop? [Y/n]: " confirm
                if [[ ! "$confirm" =~ ^[Yy]?$ ]]; then
                    log_warning "Promotion cancelled"
                    return 0
                fi
            else
                log_error "Non-interactive mode requires --yes"
                echo "Usage: bash .claude/scripts/push-dev.sh --yes"
                return 1
            fi
        fi

        log_info "Merging $original_branch into develop..."
        local merge_msg="dev: merge $original_branch into develop

Promoting $commits_ahead commit(s) from $original_branch into develop.

Generated by Dev delivery automation."

        if ! git merge "$original_branch" --no-ff -m "$merge_msg"; then
            git merge --abort >/dev/null 2>&1 || true
            git checkout "$original_branch" --quiet 2>/dev/null || true
            log_error "Merge conflict! Resolve manually:"
            echo "  1. Fix conflicts"
            echo "  2. git add ."
            echo "  3. git commit"
            echo "  4. git push origin develop"
            return 1
        fi
        log_success "Merge successful"
    fi

    sync_beads

    log_info "Pushing develop to origin..."
    if ! git push origin develop; then
        log_error "Push failed"
        echo "To retry: git push origin develop"
        return 1
    fi
    log_success "Pushed develop to origin"

    if [ "$original_branch" != "develop" ]; then
        cleanup_delivered_branch "$original_branch"
    fi

    echo ""
    echo "============================================================="
    echo "DEV DELIVERY SUCCESSFUL!"
    echo "============================================================="
    echo ""
    log_success "Delivered to develop: $original_branch -> develop"
    log_info "Current branch: $(git rev-parse --abbrev-ref HEAD)"
    echo ""

    report_stranded_commits
}

main "$@"
