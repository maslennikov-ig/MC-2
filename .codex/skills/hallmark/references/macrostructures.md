...
- **02 · Long Document** — Reads like a memo, a letter, or a journal entry. No marketing structure. Continuous prose with inline section heads. The page is _literature_ about the product. [`macrostructures/02-long-document.md`](macrostructures/02-long-document.md)
...
- **08 · Photographic** — A single huge image dominates each fold. Text is small annotation, not headline. The design says _look_ before it says _read_. [`macrostructures/08-photographic.md`](macrostructures/08-photographic.md)
...
- **19 · Map / Diagram** — A single large spatial diagram organises the page — flowchart, floor plan, network graph, system map. Information is laid out _spatially_, not linearly. [`macrostructures/19-map-diagram.md`](macrostructures/19-map-diagram.md)
...
- **Testimonials:** include the quoted person's role _and_ company. Abstract "Engineering Manager" testimonials are slop. If the brief is a real product, use real names. If the brief is a placeholder, use plausible names — never "Jane Doe" / "John Smith" (gate 20).
...
This sequence is **not** a template you stamp out — it's a recipe of _what should be present_. The macrostructure determines _how_ each section looks. A Bento Grid page interleaves features and proof inside the grid; a Stat-Led page sequences them top-to-bottom; a Marquee Hero page lets the marquee do the social-proof work.
...
4. **State your pick** in plain text _before_ writing code: "Macrostructure: Bento Grid." Then write the code, opening the CSS with the required stamp.
5. If genuinely torn, offer the user three choices from _different categories_ (e.g. Bento + Long Document + Manifesto) and let them pick.

The goal is not novelty for its own sake. The goal is that two pages Hallmark builds for two different briefs _look like different sites, not different colour-swaps of the same template_.
