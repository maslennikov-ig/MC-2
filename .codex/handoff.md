...
- Career Playbook minimal model routing is configured in Supabase: `minimax/minimax-m2.7` for `stage_career_playbook_spec` and `stage_career_playbook_judge`; `deepseek/deepseek-v4-flash` for follow-up, groups 1-6, and regenerator.
...
```text
Use $orchestrator-stage to continue Career Playbook delivery. Read AGENTS.md, .codex/orchestrator.toml, .codex/handoff.md, .codex/project-index.md, .codex/stages/mc2-db696.11/summary.md, docs/plans/quiet-waddling-starfish.md, and docs/plans/career-playbook/* first. Use Beads as source of truth. PR #24-#39 have landed in develop. Important: the primary worktree is dirty on stale feature/career-playbook-library-share; do not discard it. mc2-db696.11.5 schema/read-only/model readiness is advanced: Career Playbook tables/RLS/seeds/policies are present, preflight passes with a dedicated queue name, and model routing is MiniMax for spec/judge plus DeepSeek for the rest. Continue live mutation smoke only after auth/TOKEN, disposable fixtures, enqueuer/worker queue alignment, cleanup scope, and numeric cost budget gates are explicit. Keep billing/payment out of MVP scope.
```
