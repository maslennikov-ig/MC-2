...
| 11  | `managed_inventory_sha256`         | **PROVISIONAL** `c90edb78341fb83a6d954212daca675f5bac89f17bd5611ceb6db3e56559bac6` | 2026-07-16 owner-authorized read-only live inventory: three identical `pg_stat_activity` samples over the verify-full DSN yielded 13 stable identities (+ the probe's own contract identity appended). Canonical NFC sorted-key JSON at `deploy/qdrant/q12-managed-session-inventory.provisional.json`. Classification vocabulary (`provider-background`/`provider-client`/`application-client`/`controller-client`/`probe`), `allowed_states` incl. the `none` literal for stateless background rows, and `transaction_free_required` per identity are a documented determination PENDING independent review (same pattern as field 10); ratification must drop the `.provisional` suffix and re-record this hash. |
