...
selected_docs:
  ...
  - Repository-installed `tiktoken` 1.0.22 API/types and the encoder lifecycle pattern in `src/shared/embeddings/markdown-chunker.ts`
selected_skills:
...
verification:
  ...
  - Focused Stage 4 suite passed 117/117 across budget, preflight, durable downstream hierarchy, real Phase 2/3/4 callers, live wiring, repository, authoritative source enumeration, document preparation, and validators.
...
- Token-bound re-review proved the language-ratio estimator could undercount adversarial `cl100k_base` input (`344` actual versus `288` estimated under a `320` limit) and that `ru|en` had been passed where the display estimator expects `rus|eng`. The safety path now uses repository-installed `tiktoken` 1.0.22 with conservative `cl100k_base`: it counts the production system and serialized JSON user messages separately, adds a documented 16-token chat-envelope reserve, reuses encoders only inside bounded operations, and always frees them in `finally`. Independent EN/RU tests encode the production payload directly, cover exact and impossible framing boundaries, and prove every port call stays within the configured input limit. The tokenizer package/version/encoding/envelope are included in representation identity and every durable checkpoint; the approximate estimator remains only for display/target convergence with corrected `rus|eng` mapping.

# Verification

- `SUPABASE_URL=http://127.0.0.1:54321 SUPABASE_SERVICE_KEY=test-key pnpm exec vitest run --config vitest.config.unit.ts tests/unit/stages/stage4-analysis/evidence/budget.test.ts tests/unit/stages/stage4-analysis/evidence/preflight.test.ts tests/unit/stages/stage4-analysis/evidence/downstream-context.test.ts tests/unit/stages/stage4-analysis/evidence/downstream-phase-callers.test.ts tests/unit/stages/stage4-analysis/evidence/live-wiring.test.ts tests/unit/stages/stage4-analysis/evidence/repository.test.ts tests/unit/stages/stage4-analysis/document-source-enumeration.test.ts tests/unit/stage4-prepare-document-infos.test.ts tests/unit/stages/stage4-analysis/utils/validators.test.ts` -> 117/117.
...
- Downstream safety tokenizer: repository-installed `tiktoken` 1.0.22, `cl100k_base`, plus a conservative 16-token chat-envelope reserve. This is an input safety bound, not a claim of provider billing-token identity for unknown model tokenizers.
