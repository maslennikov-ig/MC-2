...
verification:
  ...
  - Fixed migrations/reconciliation applied gate: 12/12 on PostgreSQL 16, including runner/history, invalid-index recovery, exact trigger totals, and out-of-order latest coverage
  ...
  - Post-remediation observability/privacy gate: 74/74 passed, including aggregate-only durable totals, typed conflict usage, and invocation-only failure alert coverage
  ...
  - post-remediation course-gen-platform type-check: passed; the workspace rerun reached the untouched web package and remains red on `users-table.tsx:222` TS7006, which has no branch diff
