...
docs_review_notes: Candidate v2 requires fresh independent correctness and docs rereviews before owner approval or normative incorporation.
graph_reviewed: used
...
## Corrected D4 owner/reviewer candidate v2
...
4. addendum section 6's closed set of three added commands, only by adding
   `writers.quiesce`;
...
null on uninterrupted issuance and the prior current capability hash on an
explicit recovery reissue. All other hash fields are lower-case 64-hex. For
...
The generic launcher's `--checkpoint <hash>` and the capability field
...
| claim move, claim journal absent           | prove exact move and append claim; child could not run without `C-claimed`               |
...
exists, otherwise null. Inventory is published before the first policy mutation.
...
longest byte-exact immutable transition prefix, publishes a new capability
bound to predecessor `P`, supersedes the old capability, and journals
`quiesced/recovery_reacquired` as sole recovery issuance before claim. W may
only continue the monotonic quiesce steps; it never starts a writer. A changed,
missing, extra, or ambiguous writer/transition is an incident. If final manifest
exists but terminal transition is absent, Root reconstructs only that exact
transition from final plus the policy-no predecessor; if terminal transition
exists, Root completes the old claimed capability without overlay. Recovery
then uses `recovery_reacquired -> capability_claimed -> capability_completed ->
accepted`, one outcome per epoch.
...
Structural catalog hash is the no-LF UTF-8 `payload::text` from exact
`deploy/qdrant/q12-structural-catalog.sql` on PostgreSQL 17. The payload schema
...
Nested keys/nullability/order remain the exact fixed SQL contract. Cleanup must
...
the intent checkpoint, exact v1/archive, baseline, DB capability, and terminal
...
Command IDs are `barrier.cleanup` and `barrier.rollback`; literal argv remains
the frozen barrier command with only the exact subcommand difference. Both use
...
Order: validate predecessor/archive inputs; append/checkpoint intent; publish
rollback intent if applicable; issue/claim host capability; DB child COMMITs;
...
- quiesce recovery before final result: `recovery_reacquired ->
...
Candidate v2 must receive fresh independent correctness and documentation
...
> Подтверждаю corrected D4 v2 и оба рекомендованных варианта A: host-command capability является одним immutable файлом, перемещаемым issued/claimed/completed/superseded, с отдельными exact capability и receipt-input checkpoints; recovery никогда не переиспользует старую capability и не создаёт две authority; writers.quiesce использует exact capability/journal/checkpoint и immutable inventory/transition/final chain; DB-child публикует только terminal proof, а Root завершает host capability, удаляет DB-capability, выполняет единственный CAS receipt v1→v2 и принимает фазу; W, H и Root используют один fail-closed contract; remote/live gate не меняется.
