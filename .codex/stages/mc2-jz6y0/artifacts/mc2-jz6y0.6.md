...
verification:
  ...
  - QDRANT_URL=http://127.0.0.1:16333 QDRANT_API_KEY=test-qdrant-key pnpm --filter @megacampus/course-gen-platform test:integration:ci: passed 18 of 18 tests across 3 files; post-suite collection list empty and container removed.
  - Intended broad config run for tests/integration/qdrant.test.ts: passed 5 of 5 tests; post-suite collection list empty and container removed.
  - Exact plan broad command after required course build, isolated Redis 7, local-only Supabase placeholders, and pinned Qdrant: passed 5 of 5 tests; fixture cleanup passed and both containers were removed.
  - Configured unreachable broad suite: exited 1 from beforeAll getCollections rather than reporting a successful skipped suite.
  ...
  - Exhaustive pre-commit self-review of the four-file Q5 diff and relevant git history: no Critical, High, Medium, or Low correctness/security findings after correcting one stale fixture comment; independent reviewer still required.
changed_files:
  - .github/workflows/ci-cd.yml
  - packages/course-gen-platform/tests/integration/ci-qdrant-smoke.test.ts
  - packages/course-gen-platform/tests/integration/qdrant.test.ts
...
Only the assigned integration tests, CI workflow, isolated integration config, and this artifact changed. Production collection/upload/search modules, Compose/deploy/reindex/backup/monitoring files, stage summary, and handoff were not edited. Q5 ran in parallel with Q7 on disjoint write zones.

The default broad Vitest configuration owns BullMQ global setup and a built sandboxed processor, which is unrelated to Qdrant adapter coverage. The broad file is therefore included in `vitest.config.integration-ci.ts`, whose no-global-setup contract is the intended isolated gate. The exact plan command was also proven separately after building the processor and supplying dedicated disposable Redis, Qdrant, and local-only Supabase placeholders; no unrelated local service was used.

# Verification

The final intended CI command executed 18 tests: 9 native Qdrant correctness tests, 5 broad Qdrant adapter tests, and 4 existing schema integration tests. The explicit broad selection executed all 5 scenarios rather than an all-skipped suite. A configured closed port failed in `beforeAll`, proving that configuration makes availability blocking.
