...
verification:
  ...
  - Independent re-review 760adf03 returned 2 residual P1 findings for the external clarifying index/pgcrypto dependency and incomplete downstream absence inventory.
  - Final TDD RED: 3/19 targeted tests failed because same-name UNIQUE(id), missing pgcrypto, and an unhistoried increment function were accepted.
  - Final PostgreSQL 16 gate: 19/19 passed after exact external dependency and exhaustive downstream absence checks.
  ...
  - Security coverage: exact normalized tables, columns, constraints, internal/external indexes, triggers, RLS enable/force, policies, table ACLs, function signatures/bodies/security/search_path, function ACLs, and extensions.pgcrypto digest dependency passed tamper regressions.
...
The initial focused test failed at module resolution because the runner did not exist. After implementation, the static suite passed source digests, statement loading, target gates, confirmations, and credential-safe CLI errors. Independent review commit `47b7623e` reproduced three P1 gaps; six regression groups failed before the first remediation and passed afterwards. Re-review `760adf03` found two remaining catalog-scope gaps. The three final targeted regressions failed before implementation and passed after the bounded checks were added.

An isolated `postgres:16-alpine` container on loopback port 55432 ran all nineteen tests. In addition to the original apply/reuse/rollback/reapply and recovery paths, the suite rejects an earlier pending repository migration, unknown history, a chain gap, a later tail, 150/151 history or historyless live residue during base rollback, dropped RLS policy, changed RPC security, revoked execute grant, altered same-name evidence or clarifying indexes, disabled trigger, missing pgcrypto, an unhistoried downstream increment function, and an introduced base function left after rollback.
