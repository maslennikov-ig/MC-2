...
| `<accepted-coverage-fingerprint>` / `<accepted-coverage-run>` | accepted coverage evidence of the recovery run, computed by the emit entrypoint from the recovered `file_catalog` rows; `<accepted-coverage-run>` is the authority token `catalog:<recovery-run-id>` (owner-approved amendment 2026-07-25, superseding the `org:course:run` ledger triple; amendment `:130-131`) — read from the emitted authority/receipts, never invented |
...
`<recovery-run-id>` = UUIDv5(namespace=run-id, "q12-source-recovery"), and
`<accepted-coverage-run>` = `catalog:<that same derived recovery run id>`
