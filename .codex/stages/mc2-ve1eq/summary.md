...
- the existing non-injected workspace deployment produces a portable backend package under pnpm 10;
...
- The first delivery CI exposed pnpm 10's new `deploy` precondition in the shared API/operator
  Docker builder. `forceLegacyDeploy: true` now explicitly preserves the repository's existing
  non-injected workspace deployment; the real `pnpm --filter ... --prod deploy` reproducer changed
  from `ERR_PNPM_DEPLOY_NONINJECTED_WORKSPACE` to a portable package with runtime dependencies.
...
documentation-decision: docs-resolve - pnpm 8.15.0 L1 did not cover the Node 24 parser boundary and pnpm 10.34.5 L1 was missing; official pnpm compatibility/settings/approve-builds/deploy documentation plus direct Node 24 measurements selected pnpm 10.34.5 and the documented legacy deploy compatibility for non-injected workspaces. The exact migration facts were persisted as L2 for pnpm 10.34.5.
