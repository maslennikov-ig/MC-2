...
DO \$restore\$
DECLARE saved jsonb; job jsonb; prior_default text;
  guard_definition text; immutable_definitions text[]; definition text;
BEGIN
  ...
  -- mc2-ipwyc, measured on production 2026-07-28: the guard triggers CANNOT be dropped one by one.
  -- CREATE TRIGGER needs only the TRIGGER privilege -- which is exactly how the auth/storage
  -- relations qualify for the guarded set -- but DROP TRIGGER needs OWNERSHIP of the table, and
  -- those tables belong to supabase_auth_admin / supabase_storage_admin, not to postgres. The
  -- per-relation drops that used to stand here raised 'must be owner of relation
  -- oauth_authorizations' on the live database, which would have aborted activate PAST the point
  -- of no return. Dropping the guard FUNCTION with CASCADE removes every dependent trigger in one
  -- statement -- the relation guards, the probe guards and the q12_guard_immutable guards -- and
  -- CASCADE does not re-check the owner of each dependent object. The immutable guards must
  -- survive activation, so the function's own definition and theirs are captured from the catalog
  -- FIRST and replayed verbatim at the end of this block: no second copy of either lives in this
  -- file, so neither can drift from what install actually created.
  SELECT pg_get_functiondef(function_object.oid) INTO STRICT guard_definition
    FROM pg_proc function_object
    JOIN pg_namespace namespace ON namespace.oid=function_object.pronamespace
    WHERE namespace.nspname='q12_guard' AND function_object.proname='enforce_write_barrier';
  SELECT array_agg(pg_get_triggerdef(t.oid) ORDER BY t.oid) INTO immutable_definitions
    FROM pg_trigger t JOIN pg_class c ON c.oid=t.tgrelid
    JOIN pg_namespace n ON n.oid=c.relnamespace JOIN pg_proc p ON p.oid=t.tgfoid
    JOIN pg_namespace pn ON pn.oid=p.pronamespace
    WHERE pn.nspname='q12_guard' AND p.proname='enforce_write_barrier' AND n.nspname='q12_guard'
      AND NOT t.tgisinternal AND t.tgparentid=0
      AND t.tgname IN ('q12_guard_immutable','q12_guard_immutable_truncate');
  IF COALESCE(array_length(immutable_definitions,1),0)<>6 THEN
    RAISE EXCEPTION 'Q12 immutable guard trigger set is not exactly six before restore';
  END IF;
  DROP FUNCTION q12_guard.enforce_write_barrier() CASCADE;
  ...
  IF '$drop_schema'='false' THEN
    ...
    -- Re-arm the durable-truth guards LAST (mc2-ipwyc): the activation UPDATE above then needs no
    -- trigger exemption at all, and the activation guard immediately after this block re-proves
    -- that all six are present AND armed. On the rollback path the whole schema is dropped a few
    -- statements later, so nothing is re-created there.
    EXECUTE guard_definition;
    -- The replayed definition carries no ACL, so PUBLIC would regain EXECUTE. install revoked it
    -- with REVOKE ALL ON ALL FUNCTIONS IN SCHEMA q12_guard FROM PUBLIC, and verify_activated_state
    -- refuses any q12_guard function whose ACL names a grantee other than its owner: re-apply the
    -- same revoke so the replayed function is ACL-identical to the one install created.
    REVOKE ALL ON FUNCTION q12_guard.enforce_write_barrier() FROM PUBLIC;
    FOREACH definition IN ARRAY immutable_definitions LOOP EXECUTE definition; END LOOP;
  END IF;
END \$restore\$;
...
# mc2-ipwyc, measured against the live Supavisor pooler on 2026-07-28: the pooler does NOT deliver
# the connection's startup `options` parameter to PostgreSQL. Connecting with
# `-c default_transaction_read_only=on` left the setting at `off` -- the option has no effect at all.
# (PostgreSQL's own precedence is intact: a startup option that IS delivered beats the per-database
# setting. It simply never arrives through the pooler.) So every runner below that declares a
# read-only-ness in `options` and then PROVES it was silently proving whatever the DATABASE default
# happened to be. That was benign before install and fatal after it: the main runner's
# `transaction_read_only='off'` session proof cannot pass once install has set the database default
# read-only, and the terminal proof's `='on'` cannot pass once restore has put the default back.
# Each runner now states its intent with a session-level SET (the GUC is USERSET and the production
# DSN is session-mode port 5432) and keeps the proof as the post-condition -- which still fails
# closed on a genuinely read-only server, where the SET cannot take effect.
