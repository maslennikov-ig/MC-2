...
import { normalizePhase1Output, quickValidatePhase1Structure } from '../src/structure-normalizer';
...
describe('quickValidatePhase1Structure', () => {
  ...
  it('returns error when course_category is missing', () => {
    ...
    expect(errors.some((e: string) => e.includes('course_category'))).toBe(true);
  });
  ...
  it('returns error when topic_analysis is missing', () => {
    ...
    const errors = quickValidatePhase1Structure(data);
    expect(errors.some((e: string) => e.includes('topic_analysis'))).toBe(true);
  });
  ...
  it('returns error when course_category.primary is missing', () => {
    ...
    const errors = quickValidatePhase1Structure(data);
    expect(errors.some((e: string) => e.includes('primary'))).toBe(true);
  });
