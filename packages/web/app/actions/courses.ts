...
const courseSchema = z.object({
  topic: z
    .string()
    .min(3, 'Название курса должно быть не менее 3 символов')
    .max(200, 'Название курса должно быть менее 200 символов'),
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        ...
        return {
          success: false,
          error:
            'Достигнут лимит одновременных генераций. Пожалуйста, дождитесь завершения текущих курсов.',
...
export async function updateDraftAndStartGeneration(
  ...
  try {
    ...
    const validationResult = courseSchema.safeParse({
      ...
      output_formats:
        formData.getAll('output_formats').length > 0
    ...
    logger.info('Course draft updated, awaiting user approval to start generation', {
      courseId: course.id,
      slug: course.slug,
    })
 ...
 */
export async function cancelCourseGeneration(
  courseId: string
