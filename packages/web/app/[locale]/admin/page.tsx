...
export default async function AdminPage({ params }: Props) {
  ...
  const [usersTotal, usersNewWeek, coursesTotal, lessonsTotal, jobStats, errorsLast24h] =
    await Promise.all([
  ...
  const activeJobs = jobStats.data?.filter((j) => activeStatuses.includes(j.status)).length || 0
