...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
      ...
      logPermanentFailure({
        error_message: 'Login failed: Supabase environment variables not set',
    ...
    if (error) {
      ...
      logPermanentFailure({
        error_message: error.message || 'Login authentication failed',
        stack_trace: undefined,
        severity: 'ERROR',
        job_type: 'AUTH_LOGIN',
        metadata: {
    ...
    supabaseCookies.forEach((cookie) => {
      ...
      response.cookies.set(cookie.name, cookie.value, COOKIE_CONFIG.auth())
    })
    ...
    logger.error('Login error:', error)
    logPermanentFailure({
      error_message: error instanceof Error ? error.message : 'Internal server error during login',
