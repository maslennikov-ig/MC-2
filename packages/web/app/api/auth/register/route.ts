...
async function checkRateLimit(identifier: string): Promise<boolean> {
  ...
  try {
    ...
    if (error) {
      ...
      if (
        (error as Error & { code?: string }).code === 'PGRST202' ||
        error.message?.includes('function') ||
        error.message?.includes('does not exist')
      ) {
        logger.warn('Rate limit function not found in database - skipping rate limiting', {
          error: error.message,
        })
        return true
      }
    ...
    logPermanentFailure({
      error_message: err instanceof Error ? err.message : 'Unexpected error in rate limit check',
...
export async function POST(req: Request) {
  try {
    ...
    logPermanentFailure({
      error_message:
        error instanceof Error ? error.message : 'Internal server error during registration',
      stack_trace: error instanceof Error ? error.stack : undefined,
      severity: 'ERROR',
      job_type: 'AUTH_REGISTER',
      metadata: {
