...
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    logger.info('Share API: User authenticated', {
      requestId,
      userId: user.id,
    ...
    if (!courseData) {
      logger.warn('Share API: Course not found', { requestId, orgSlug, courseSlug })
      return NextResponse.json(
        {
          error: 'Not found',
          message: `Course not found in organization`,
          requestId,
        },
        { status: 404 }
      )
    }
    logger.info('Share API: Course found', { requestId, courseId: courseData.id })
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    if (!courseData) {
      logger.warn('Share API DELETE: Course not found', { requestId, orgSlug, courseSlug })
      return NextResponse.json(
        {
          error: 'Not found',
          message: `Course not found in organization`,
