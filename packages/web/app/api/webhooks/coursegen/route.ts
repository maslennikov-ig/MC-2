    ...
    switch (payload.status) {
      ...
      case 'processing': {
        ...
        if (payload.progress) {
          ...
          if (course?.generation_progress) {
            ...
            updates.generation_progress =
              progress as unknown as Database['public']['Tables']['courses']['Update']['generation_progress']
          }
        }

        const { error } = await supabase.from('courses').update(updates).eq('id', payload.courseId)
        ...
        if (error) {
          ...
          logPermanentFailure({
            error_message: `Failed to update course progress: ${error.message}`,
            severity: 'ERROR',
            job_type: 'WEBHOOK_COURSEGEN',
            metadata: {
      ...
      case 'completed': {
        ...
        if (error) {
          ...
          // Log to error_logs for admin visibility
          logPermanentFailure({
            error_message: `Failed to mark course as completed: ${error.message}`,
            severity: 'ERROR',
            job_type: 'WEBHOOK_COURSEGEN',
            metadata: {
              route: '/api/webhooks/coursegen',
      ...
      case 'failed': {
        ...
        if (error) {
          ...
          // Log to error_logs for admin visibility
          logPermanentFailure({
            error_message: `Failed to mark course as failed: ${error.message}`,
            severity: 'ERROR',
            job_type: 'WEBHOOK_COURSEGEN',
            metadata: {
              route: '/api/webhooks/coursegen',
        ...
        // Log the course failure itself to error_logs
        logPermanentFailure({
          error_message: payload.error || 'Course generation failed',
          severity: 'ERROR',
