    ...
    logPermanentFailure({
      ...
      error_message:
        error instanceof Error
          ? error.message
          : 'Unexpected error in GET /api/organizations/[orgId]/invitations',
  ...
  try {
    ...
    if (!hasAccess && user.role !== 'superadmin') {
      return NextResponse.json(
        {
          error: 'Forbidden',
          message: 'Only organization owners and admins can create invitations',
          requestId,
        },
        { status: 403 }
      )
    ...
    if (error) {
      ...
      logPermanentFailure({
        user_id: user.id,
        organization_id: orgId,
        error_message: error.message || 'Database error creating invitation',
        stack_trace: undefined,
        severity: 'ERROR',
        job_type: 'ORG_INVITE_CREATE',
        metadata: {
          route: `/api/organizations/${orgId}/invitations`,
    ...
    logPermanentFailure({
      ...
      error_message:
        error instanceof Error
          ? error.message
          : 'Unexpected error in POST /api/organizations/[orgId]/invitations',
      stack_trace: error instanceof Error ? error.stack : undefined,
      severity: 'ERROR',
      job_type: 'ORG_INVITE_CREATE',
      metadata: {
