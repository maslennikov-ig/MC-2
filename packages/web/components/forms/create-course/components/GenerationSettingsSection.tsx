...
export function GenerationSettingsSection({
  ...
  // Course size for token estimation
  const courseSize = watch('courseSize')
  const estimatedLessons = watch('estimatedLessons')
  ...
  return (
    <motion.div
      ...
      className="overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-br from-white/90 to-white/70 p-6 backdrop-blur-xl md:p-8 xl:col-span-2 dark:border-white/10 dark:from-black/70 dark:to-black/60"
    >
      {/* Section Header */}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        {/* Right column - Generation Mode + Token Estimate */}
        <div className="space-y-6">
          {/* Generation Mode Section */}
          <div>
            <div className="mb-4 flex items-center gap-3">
              <Zap className="h-5 w-5 text-purple-500 dark:text-purple-400" />
              <span className="font-medium text-slate-900 dark:text-white">Режим генерации</span>
            </div>
          ...
          {/* Token Estimate Card (visible when automatic) */}
          <TokenEstimateCard
            documentCount={documentCount}
            estimatedLessons={estimatedLessons}
            courseSize={courseSize}
            hasDocuments={hasDocuments}
            isVisible={isAutomatic}
          />
        </div>
      </div>
    </motion.div>
