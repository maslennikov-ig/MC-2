...
describe('MarkdownRenderer - Contract Tests', () => {
  describe('Basic markdown rendering', () => {
    it('should render heading (h1)', async () => {
      const result = await renderAsync(MarkdownRenderer({ content: '# Hello World' }))
...
- List item 2
  ...
  describe('Preset application', () => {
    it('should apply default "lesson" preset className', async () => {
      const result = await renderAsync(MarkdownRenderer({ content: '# Test' }))
    ...
    it('should apply "chat" preset className', async () => {
      const result = await renderAsync(MarkdownRenderer({ content: '# Test', preset: 'chat' }))
    ...
    it('should apply "preview" preset className', async () => {
      const result = await renderAsync(MarkdownRenderer({ content: '# Test', preset: 'preview' }))
    ...
    it('should apply "minimal" preset className', async () => {
      const result = await renderAsync(MarkdownRenderer({ content: '# Test', preset: 'minimal' }))
  ...
  describe('Wrapper element', () => {
    ...
    it('should contain rendered markdown inside article', async () => {
      const result = await renderAsync(MarkdownRenderer({ content: '# Nested Content' }))
  ...
  describe('Trust mode handling', () => {
    it('should accept trusted=true (default)', async () => {
      await expect(
        renderAsync(MarkdownRenderer({ content: '# Test', trusted: true }))
      ).resolves.not.toThrow()
    })

    it('should accept trusted=false (untrusted mode)', async () => {
      await expect(
        renderAsync(MarkdownRenderer({ content: '# Test', trusted: false }))
      ).resolves.not.toThrow()
    })
    ...
    it('should render content in both trust modes', async () => {
      ...
      const trustedResult = await renderAsync(MarkdownRenderer({ content, trusted: true }))
      const untrustedResult = await renderAsync(MarkdownRenderer({ content, trusted: false }))
  ...
  describe('Feature overrides', () => {
    ...
    it('should render with empty features object', async () => {
      await expect(
        renderAsync(MarkdownRenderer({ content: '# Test', features: {} }))
      ).resolves.not.toThrow()
    })
  })
  ...
  describe('Custom components', () => {
    it('should accept custom components prop', async () => {
      await expect(
        renderAsync(MarkdownRenderer({ content: '# Test', components: {} }))
      ).resolves.not.toThrow()
    })

    it('should render with empty components object', async () => {
      const result = await renderAsync(MarkdownRenderer({ content: '# Test', components: {} }))
  ...
  describe('Edge cases', () => {
    it('should handle very long content', async () => {
      ...
      await expect(renderAsync(MarkdownRenderer({ content: longContent }))).resolves.not.toThrow()

      const result = await renderAsync(MarkdownRenderer({ content: longContent }))
      expect(result).not.toBeNull()
    })
