...
export function StageResultsDrawer({
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent
        side="right"
        className="flex w-full max-w-[50vw] flex-col border-l border-gray-800 bg-[#0a0e1a] p-0 text-gray-100"
      >
        ...
        <Tabs defaultValue="results" className="flex flex-1 flex-col overflow-hidden">
          <div className="px-6 pt-2">
            <TabsList className="w-full border border-gray-800 bg-gray-900">
              <TabsTrigger
                value="results"
          ...
          <div className="flex-1 overflow-y-auto">
            <TabsContent value="results" className="h-full p-6 pt-4">
              {stageNumber === 3 ? (
                <PrioritizationView courseId={courseId} editable={true} onApproved={onClose} />
              ) : stageNumber ? (
