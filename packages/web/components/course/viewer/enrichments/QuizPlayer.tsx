...
export function QuizPlayer({ content, enrichmentId, onComplete }: QuizPlayerProps) {
  ...
  const getDifficultyLabel = (difficulty: string) => {
    switch (difficulty) {
      case 'easy':
        return t('viewer.difficulty.easy')
      case 'medium':
        return t('viewer.difficulty.medium')
      case 'hard':
        return t('viewer.difficulty.hard')
      default:
        return difficulty
    }
  }
  ...
  const getBloomLabel = (level: string) => {
    switch (level) {
      case 'remember':
        return t('viewer.bloom.remember')
      case 'understand':
        return t('viewer.bloom.understand')
      case 'apply':
        return t('viewer.bloom.apply')
      case 'analyze':
        return t('viewer.bloom.analyze')
      default:
        return level
    }
  }
        ...
        <Card className="border-2">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold">{t('viewer.questionsReview')}</h3>
              {questions.map((question, index) => {
                const correct = isAnswerCorrect(question)
                return (
                  <div key={question.id} className="rounded-lg border p-4 dark:border-gray-700">
    ...
    <div className="relative space-y-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <CardTitle className="mb-2 text-2xl">{content.quiz_title}</CardTitle>
              <CardDescription className="text-base">{content.instructions}</CardDescription>
            </div>
            {content.time_limit_minutes && (
              <Badge variant="outline" className="ml-4 gap-1.5">
      ...
      <div>
        ...
        <Progress
          ...
          aria-valuetext={t('viewer.questionOf', {
            current: state.currentQuestionIndex + 1,
            total: totalQuestions,
          })}
        />
          ...
          <Card className="border-2">
            <CardHeader>
              <div className="mb-3 flex flex-wrap gap-2">
                ...
                <Badge variant="outline">
                  {t('viewer.pointsEarned', { count: currentQuestion.points })}
                </Badge>
              </div>
