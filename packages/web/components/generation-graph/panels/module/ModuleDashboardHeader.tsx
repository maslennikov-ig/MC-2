  ...
  return (
    <svg width={size} height={size} className="-rotate-90 transform">
      ...
      <circle
        ...
        strokeLinecap="round"
        className="text-blue-500 transition-all duration-500 dark:text-cyan-400"
      />
    </svg>
  )
}
 ...
 */
export function ModuleDashboardHeader({ data, className }: ModuleDashboardHeaderProps) {
  const { title, moduleNumber, status, aggregates } = data

  // Calculate progress percentage
  const progressPercent =
    aggregates.totalLessons > 0
      ? Math.round((aggregates.completedLessons / aggregates.totalLessons) * 100)
      : 0

  // Get quality color if available
  const qualityColor =
    aggregates.avgQualityScore !== null ? getQualityColor(aggregates.avgQualityScore) : null
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <div
            className={cn(
              ...
              'hover:border-blue-300 hover:shadow-md dark:hover:border-cyan-600'
            )}
          >
            <div className="mb-3 flex items-center gap-2">
              <CheckCircle2 className="h-4 w-4 text-blue-500 dark:text-cyan-400" />
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                <div className="font-mono text-xl font-semibold text-slate-900 dark:text-cyan-400">
                  {aggregates.completedLessons}/{aggregates.totalLessons}
                </div>
                <div className="text-xs text-slate-500 dark:text-slate-400">{progressPercent}%</div>
              </div>
            </div>
          </div>
          ...
          >
            <div className="mb-3 flex items-center gap-2">
              <DollarSign className="h-4 w-4 text-blue-500 dark:text-cyan-400" />
              <div className="text-xs font-medium tracking-wide text-slate-600 uppercase dark:text-slate-400">
                Стоимость
              </div>
            </div>
            <div className="font-mono text-2xl font-semibold text-slate-900 dark:text-cyan-400">
          ...
          >
            <div className="mb-3 flex items-center gap-2">
              <TrendingUp className="h-4 w-4 text-blue-500 dark:text-cyan-400" />
              <div className="text-xs font-medium tracking-wide text-slate-600 uppercase dark:text-slate-400">
                Качество
              </div>
            </div>
          ...
          >
            <div className="mb-3 flex items-center gap-2">
              <Clock className="h-4 w-4 text-blue-500 dark:text-cyan-400" />
              <div className="text-xs font-medium tracking-wide text-slate-600 uppercase dark:text-slate-400">
                Время
              </div>
            </div>
