...
function DocumentCandidateItem({ document }: DocumentCandidateItemProps) {
  ...
  return (
    <div className="bg-muted/30 hover:bg-muted/50 space-y-2 rounded-md p-3 transition-colors">
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span className="text-muted-foreground text-xs">{formatFileSize(document.fileSize)}</span>
        </div>
      </div>
      ...
      {summaryPreview && (
        <div className="border-l-2 border-amber-500/30 pl-2">
          <p className="text-muted-foreground line-clamp-2 text-xs italic">{summaryPreview}</p>
        </div>
      )}
    </div>
  )
...
export const Stage3InputTab = memo<Stage3InputTabProps>(function Stage3InputTab({
  ...
  const [courseContext, setCourseContext] = useState<{ title: string; description: string } | null>(
    null
  )
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const { data: filesData, error: filesError } = await supabase
          .from('file_catalog')
          .select(
            'id, filename, original_name, file_size, mime_type, summary_metadata, processed_content'
          )
    ...
    <div className="grid grid-cols-5 gap-4 p-4">
      ...
      <Card className="col-span-3">
        <CardHeader className="pb-3">
          ...
          <p className="text-muted-foreground mt-1 text-xs">{t('sortedByTokens')}</p>
        </CardHeader>
        <CardContent>
          <ScrollArea className="h-[280px] pr-4">
            <div className="space-y-2">
              {isLoading ? (
                ...
                documents.map((doc) => <DocumentCandidateItem key={doc.id} document={doc} />)
              ) : (
                <div className="text-muted-foreground py-8 text-center text-sm">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="text-muted-foreground">{t('tokenBudget')}</span>
          ...
          <div className="border-border/50 border-t pt-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="text-center">
                ...
                <p className="text-muted-foreground text-xs">{t('candidatesCount')}</p>
              </div>
              <div className="text-center">
                <span className="text-2xl font-bold">
                  {documents.filter((d) => d.summaryTokens > HEAVY_TOKEN_THRESHOLD).length}
                </span>
                <p className="text-muted-foreground text-xs">{t('heavyDocument')}</p>
              </div>
            </div>
          </div>
        </CardContent>
