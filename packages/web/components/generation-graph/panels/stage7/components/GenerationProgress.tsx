    ...
    <div className={cn('flex h-full flex-col', className)}>
      ...
      <div className="border-b p-4">
        <div className="flex items-center gap-3">
          {isActive && <Loader2 className="text-primary h-5 w-5 animate-spin" />}
      ...
      >
        ...
        <div className="space-y-1">
          {logs.length === 0 ? (
            <div className="text-slate-500">{t('generationLog.waiting')}</div>
          ) : (
            logs.map((log, idx) => (
              <div key={idx} className="text-slate-300">
                <span className="text-slate-500">[{String(idx + 1).padStart(2, '0')}]</span> {log}
              </div>
            ))
          )}

          {isActive && <div className="animate-pulse text-green-400">▌</div>}
        </div>
      </div>
    </div>
  )
