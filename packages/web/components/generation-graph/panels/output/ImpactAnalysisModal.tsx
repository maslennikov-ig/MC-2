...
const translations = {
  ru: {
    ...
    criticalDescription:
      'Это изменение затронет очень большое количество элементов. Рекомендуется выбрать "Пометить для проверки" вместо автоматической перегенерации.',
    impactLevel: {
      low: 'Низкое влияние',
  ...
  en: {
    ...
    criticalDescription:
      'This change will affect a very large number of elements. Consider marking for review instead of auto-regeneration.',
    impactLevel: {
      low: 'Low Impact',
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={cn('sm:max-w-[540px]', styles.containerClass)}>
        ...
        <div className="space-y-4 pt-2">
          ...
          {impactLevel === 'critical' && (
            <div className="rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-900/20">
              <div className="flex items-center gap-2 text-red-700 dark:text-red-400">
                ...
                <span className="text-lg font-bold">{t.criticalWarning}</span>
              </div>
              <p className="mt-2 text-sm text-red-600 dark:text-red-300">{t.criticalDescription}</p>
            </div>
          )}

          {/* High Impact Warning */}
          {impactLevel === 'high' && (
            <div className="rounded-md border border-orange-200 bg-orange-50 p-3 dark:border-orange-800 dark:bg-orange-900/10">
          ...
          {impactLevel === 'medium' && (
            <div className="rounded-md border border-yellow-200 bg-yellow-50 p-3 dark:border-yellow-800 dark:bg-yellow-900/10">
          ...
          {/* Field Info */}
          <div className="space-y-1 rounded-md bg-slate-50 p-3 dark:bg-slate-800/50">
            <p className="text-xs font-medium text-slate-500 dark:text-slate-400">{t.fieldLabel}</p>
            <p className="text-sm font-medium text-slate-900 dark:text-slate-100">{fieldLabel}</p>
            <p className="font-mono text-xs text-slate-500 dark:text-slate-400">{fieldPath}</p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <RadioGroup
              ...
              aria-label={
                locale === 'ru'
                  ? 'Выберите действие для каскадного обновления'
                  : 'Cascade action options'
              }
            >
              {/* Mark Stale (Safest) */}
              <div className="flex items-start space-x-3 rounded-md border-2 border-slate-200 p-3 transition-colors hover:border-slate-300 dark:border-slate-700 dark:hover:border-slate-600">
                <RadioGroupItem
                  value="mark_stale"
                  id="mark_stale"
                  className="mt-0.5"
                  aria-describedby="action-desc-mark-stale"
                />
                <div className="flex-1 space-y-1">
                  <Label htmlFor="mark_stale" className="cursor-pointer text-sm font-medium">
                    {t.actions.mark_stale.label}
                  </Label>
                  <p
                    id="action-desc-mark-stale"
              ...
              <div
                className={cn(
                  ...
                  selectedAction === 'auto_regenerate'
                    ? impactLevel === 'critical'
                      ? 'animate-pulse border-red-400 bg-red-50 dark:border-red-600 dark:bg-red-900/10'
              ...
              >
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <p
                    id="action-desc-auto-regenerate"
                    className="text-xs text-slate-500 dark:text-slate-400"
                  >
              ...
              <div className="flex items-start space-x-3 rounded-md border-2 border-slate-200 p-3 transition-colors hover:border-slate-300 dark:border-slate-700 dark:hover:border-slate-600">
                <RadioGroupItem
                  ...
                  id="review_each"
                  className="mt-0.5"
                  aria-describedby="action-desc-review-each"
                />
                <div className="flex-1 space-y-1">
                  <Label htmlFor="review_each" className="cursor-pointer text-sm font-medium">
                    {t.actions.review_each.label}
                  </Label>
                  <p
                    id="action-desc-review-each"
                    className="text-xs text-slate-500 dark:text-slate-400"
                  >
