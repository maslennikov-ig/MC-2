...
import { Clock, Target, BookMarked, ChevronDown, Trash2 } from 'lucide-react'
 ...
 * title, duration, objectives, and topics.
 *
 * Layout:
...
export const LessonRow = ({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(
        locale === 'ru' ? `Урок ${lessonNumber} удален` : `Lesson ${lessonNumber} deleted`
      )

      // Parent component will handle UI update via revalidation
    } catch (error) {
      toast.error(locale === 'ru' ? 'Не удалось удалить урок' : 'Failed to delete lesson')
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className="cursor-pointer space-y-2 rounded-md px-4 py-3 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50"
          onClick={(e) => {
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <Badge
              variant="outline"
              className="shrink-0 bg-white text-xs text-slate-700 dark:bg-slate-800 dark:text-slate-300"
            >
              {lessonNumber}
            </Badge>
            ...
            <div
              ...
              {...(canEdit && isFirstLesson ? { 'data-auto-focus-target': 'true' } : {})}
            >
              {canEdit ? (
                <EditableField
                  config={titleConfig}
            ...
            <div className="shrink-0">
              {canEdit ? (
                ...
                <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                  ...
                  <span>
                    {lesson.estimated_duration_minutes} {t.minutes}
                  </span>
                </div>
              )}
            </div>
        ...
        {isExpanded && (
          <div
            ...
            className="space-y-3 border-t border-slate-100 bg-slate-50/50 px-4 py-3 dark:border-slate-700 dark:bg-slate-900/50"
            role="region"
          ...
          >
              ...
              lesson.lesson_objectives.length > 0 && (
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <h4 className="text-xs font-medium tracking-wide text-slate-700 uppercase dark:text-slate-300">
                      {t.objectives}
                    </h4>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {lesson.lesson_objectives.map((objective, idx) => (
                      <Badge
                        ...
                        variant="secondary"
                        className="border-blue-200 bg-blue-50 text-xs text-blue-700 dark:border-blue-800 dark:bg-blue-900/30 dark:text-blue-300"
                      >
                        {objective}
                      </Badge>
                    ))}
            ...
            {/* practical_exercises display REMOVED — Stage 6 generates exercises from content */}
          </div>
        )}
      </div>
