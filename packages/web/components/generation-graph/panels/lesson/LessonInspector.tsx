 ...
 */
function InspectorErrorFallback({
  error,
  resetErrorBoundary,
}: {
  error: Error
...
export function LessonInspector({
    ...
    (pipelineNodes: PipelineNodeState[]): Record<Stage6NodeName, number> | undefined => {
      ...
      for (const node of pipelineNodes) {
        if (node.tokensUsed && node.tokensUsed > 0) {
          breakdown[node.node] = node.tokensUsed
          hasTokens = true
        }
      }
  ...
  if (isLoading) {
    return (
      <div className={cn('flex h-full flex-col items-center justify-center py-12', className)}>
        ...
        <p className="mt-4 text-sm text-slate-500 dark:text-slate-400">Загрузка данных урока...</p>
      </div>
    )
  }
  ...
  if (!data) {
    return (
      <div className={cn('flex h-full flex-col items-center justify-center py-12', className)}>
        <p className="text-slate-500 dark:text-slate-400">Данные урока не найдены</p>
      </div>
    )
  }
  ...
  const getErrorMessage = (): string | undefined => {
    ...
    if (errorNode?.errorMessage) {
      ...
      const nodeLabel = STAGE6_NODE_LABELS[errorNode.node]?.ru || errorNode.node
      ...
      >
        <DialogContent className="flex max-h-[80vh] max-w-3xl flex-col overflow-hidden">
          ...
          <div className="mt-4 flex-1 overflow-y-auto">
            <pre className="overflow-x-auto rounded border border-slate-200 bg-slate-100 p-4 text-xs break-words whitespace-pre-wrap text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
