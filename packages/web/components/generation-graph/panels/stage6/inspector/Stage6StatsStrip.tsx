    ...
    if (!hasBreakdown) {
      return (
        <div className="flex items-center gap-1.5 text-sm">
          ...
          <span className="text-muted-foreground">{locale === 'ru' ? 'токенов' : 'tokens'}</span>
        </div>
      )
    }
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <div className="flex cursor-help items-center gap-1.5 text-sm">
              <Coins className="h-3.5 w-3.5 text-cyan-600 dark:text-cyan-400" />
              <span className="font-medium">{formatTokensCompact(tokens)}</span>
              ...
              {Object.entries(tokensBreakdown).map(([nodeName, nodeTokens]) => {
                ...
                const label =
                  locale === 'ru'
                    ...
                    : nodeName
                return (
                  <div key={nodeName} className="flex items-center justify-between gap-3 text-sm">
                    <span className="text-muted-foreground">{label}:</span>
                    <span className="font-mono font-medium">{formatTokensCompact(nodeTokens)}</span>
                  </div>
                )
              })}
            </div>
...
export const Stage6StatsStrip = memo(
  ({
    ...
    return (
      <div className="sticky top-0 z-10 flex items-center gap-4 border-b border-slate-200 bg-slate-50/95 px-4 py-2 backdrop-blur dark:border-slate-700 dark:bg-slate-900/95">
        <TokensMetric tokens={tokens} tokensBreakdown={tokensBreakdown} locale={locale} />
