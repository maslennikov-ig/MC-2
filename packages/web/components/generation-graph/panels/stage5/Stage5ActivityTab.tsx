...
function ActorIcon({ actor }: { actor: ActivityActor }) {
  switch (actor) {
    ...
    case 'ai':
      return (
        <Brain className="h-3.5 w-3.5 text-orange-600 dark:text-orange-400" aria-hidden="true" />
      )
    default:
...
const ActivityEventRow = memo<ActivityEventRowProps>(function ActivityEventRow({
  ...
  const deltaStr = deltaMs !== undefined && deltaMs > 0 ? `+${(deltaMs / 1000).toFixed(1)}s` : null
  ...
  return (
    <div
      className={cn(
        ...
        isProgress && 'border-l-2 border-orange-500 bg-orange-50/70 dark:bg-orange-950/20'
      )}
    >
      {/* Status Icon */}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <span
            className={cn(
              'text-sm',
              isProgress ? 'font-medium text-orange-900 dark:text-orange-100' : 'text-foreground'
            )}
          >
            {event.message}
          </span>
  ...
  if (events.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        <Activity
          className="mb-4 h-12 w-12 text-orange-300 dark:text-orange-700"
          aria-hidden="true"
        />
              ...
              >
                <AccordionTrigger className="hover:bg-muted/50 px-4 py-3 hover:no-underline">
                  <div className="flex w-full items-center gap-3">
                    <PhaseIcon
                      className={cn('h-4 w-4', PHASE_GROUP_COLORS[phase])}
                      aria-hidden="true"
                    />
                    ...
                    <div className="space-y-1">
                      {phaseEvents.map((event, index) => (
                        <ActivityEventRow
                          ...
                          locale={locale}
                          previousEventTime={
                            index > 0 ? phaseEvents[index - 1].timestamp : undefined
                          }
                        />
                      ))}
                    </div>
...
function mapPhaseGroupFromTrace(trace: GenerationTrace): Stage5ActivityPhaseGroup {
  ...
  if (
    content.includes('validate_input') ||
  ...
  if (
    ...
    content.includes('generate_sections') ||
  ...
  if (
    ...
    content.includes('validate_quality') ||
    content.includes('embedding')
  ) {
    return 'quality'
  }
  if (
    content.includes('finalization') ||
    content.includes('validate_lessons') ||
