...
export function IterationProgressChart({
  ...
  const targetY = padding.top + chartHeight - ((targetScore - minScore) / scoreRange) * chartHeight
  ...
  return (
    <TooltipProvider>
      <div className={cn('w-full max-w-sm', className)}>
        ...
        {/* Legend */}
        <div className="mt-1 flex items-center justify-between px-2 text-xs text-slate-500 dark:text-slate-400">
          ...
          {iterations.length > 0 && iterations[iterations.length - 1].endScore !== null && (
            <span>
              ...
              <span
                className={cn(
                  getScoreColor(iterations[iterations.length - 1].endScore!, targetScore)
                )}
              >
