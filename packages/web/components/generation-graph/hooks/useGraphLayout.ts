...
export function useGraphLayout() {
   ...
   */
  const toElkGraph = useCallback(
    (nodes: AppNode[], edges: AppEdge[]): ElkGraph => {
      ...
      const getModuleDimensions = (node: AppNode) => {
        if (node.type === 'module' && node.data) {
          ...
          const totalLessons = node.data.totalLessons || 0
        ...
        if (node.type === 'stage2group' && node.data) {
          const isCollapsed = node.data.isCollapsed ?? false
          const totalDocuments = node.data.totalDocuments || 0
      ...
      topLevelNodes.forEach((node, index) => {
        ...
        } else if ((node.type as string) === 'clarifying') {
          // Clarifying node: position BELOW Stage 4, not in main flow
          // Low priority keeps it out of the horizontal main pipeline
          nodeLayoutOptions = {
            'elk.alignment': 'BOTTOM',
            'elk.layered.priority': '100', // Low priority - side branch
          }
        }
  ...
  const layoutNodes = useCallback(
    async (
      ...
      const hasPositions = layoutedGraph.children?.some(
      ...
      const stageNodes = layoutedTopLevelRaw.filter(
        (n) => n.type === 'stage' || n.type === 'stage2group'
      )
      const otherNodes = layoutedTopLevelRaw.filter(
        (n) => n.type !== 'stage' && n.type !== 'stage2group'
      )
      ...
      if (moduleNodes.length > 1) {
        ...
        const alignedModules = sortedModules.map((module, index) => {
          ...
          const prevModulesHeight = sortedModules.slice(0, index).reduce((sum, m) => {
