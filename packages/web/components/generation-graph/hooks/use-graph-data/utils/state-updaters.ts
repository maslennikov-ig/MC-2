...
export function updateDocumentSteps(
  ...
  newTraces.forEach((trace) => {
    ...
    if (docId) {
      ...
      const existingStepAttempts =
        attemptsUpdates.get(stepNodeId) || attemptsMap.get(stepNodeId) || []
      ...
      if (existingDoc) {
        ...
        const displayName =
          docName || getFilename?.(docId) || `Документ ${docId.substring(0, 8)}...`
        nextDocs.set(docId, {
          id: docId,
          name: displayName,
          steps: [newStep],
...
export function updateParallelItems(
  ...
  newTraces.forEach((trace) => {
    ...
    if (trace.stage === 'stage_5') {
      ...
      // Type for Stage 5 output data structure
      type Stage5Output = {
        modules?: unknown
        course_structure?: {
          sections?: { section_title?: string; lessons?: { lesson_title?: string }[] }[]
        }
      }
      const outputData = trace.output_data as Stage5Output | undefined

      if (outputData?.modules) {
        const rawModules = outputData.modules
        if (isValidModuleStructure(rawModules)) {
          modules = rawModules
        }
      } else if (outputData?.course_structure?.sections) {
        const sections = outputData.course_structure.sections
