...
export function extractDocumentId(trace: GenerationTrace): string | null {
  const inputData = trace.input_data as Record<string, string | undefined> | undefined
...
export function extractDocumentName(trace: GenerationTrace): string | null {
  ...
  // 2. File name fields
  const fileName = inputData.file_name || inputData.filename || inputData.fileName || inputData.name
  ...
  const path = inputData.filePath || inputData.file_path || inputData.path || inputData.source_file
