...
export const AssetDock: React.FC<AssetDockProps> = ({
  ...
  if (zoom < 0.4) {
    const content = (
      <div
        className={`h-1.5 w-1.5 rounded-full transition-all duration-300 ${getDotColor()} ${isGenerating ? 'animate-pulse' : ''} `}
    ...
    return onClick ? (
      <button onClick={handleClick} className="flex h-[14px] items-center justify-center">
        {content}
      </button>
    ) : (
      <div className="flex h-[14px] items-center justify-center">{content}</div>
    )
  }
  ...
  if (zoom < 0.6) {
    const content = (
      <div
        className={`rounded border px-1.5 py-0.5 text-[10px] font-medium transition-all duration-300 ${getBadgeColor()} ${isGenerating ? 'animate-pulse' : ''} `}
        title={enrichmentCountText}
        role="img"
        aria-label={enrichmentCountText}
      >
  ...
  const sortedEnrichments = [...enrichments].sort(
    (a, b) =>
      ENRICHMENT_TYPE_CONFIG[a.type as EnrichmentType].order -
      ENRICHMENT_TYPE_CONFIG[b.type as EnrichmentType].order
  )
  ...
  const iconsContent = (
    <>
      {sortedEnrichments.slice(0, 5).map((enrichment) => {
        ...
        return (
          <div key={enrichment.type} title={tooltipText} role="img" aria-label={tooltipText}>
            <Icon
              className={`h-3.5 w-3.5 transition-all duration-300 ${getIconColor()} ${statusConfig.animate ? 'animate-pulse' : ''} `}
            />
          </div>
        )
      })}
  ...
  return onClick ? (
    <button
      onClick={handleClick}
      className="flex h-[14px] items-center gap-1 rounded px-1 transition-colors hover:bg-slate-100 dark:hover:bg-slate-800"
    >
      {iconsContent}
    </button>
  ) : (
    <div className="flex h-[14px] items-center gap-1 px-1">{iconsContent}</div>
  )
}
