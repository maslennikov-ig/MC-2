...
function CompactStat({ icon, value, tooltip, className, container }: CompactStatProps) {
  return (
    <Tooltip>
      ...
      <TooltipContent side="bottom" className="px-2 py-1 text-[10px]" container={container}>
        {tooltip}
      </TooltipContent>
    </Tooltip>
  )
...
function Separator({ isDark }: { isDark?: boolean }) {
  return (
    <span className={isDark ? 'text-slate-600 select-none' : 'text-slate-300 select-none'}>│</span>
  )
}
...
function MobileStat({ label, value, subValue, icon, isDark }: MobileStatProps) {
  return (
    <div className={cn('rounded px-1.5 py-1', isDark ? 'bg-slate-800/50' : 'bg-slate-100')}>
...
export const GraphHeader = ({
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div
        className={cn(
          'relative z-20 overflow-hidden border-b shadow-sm',
      ...
      >
        ...
        <motion.div layout className="flex items-center gap-2 px-3 py-1.5">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <div className="relative flex-shrink-0">
              <Rocket className={`h-3.5 w-3.5 ${isDark ? 'text-purple-400' : 'text-purple-500'}`} />
              {!isConnected && (
            ...
            <span
              className={cn(
                'max-w-[300px] truncate text-xs font-medium',
          ...
          <div className="hidden h-1.5 flex-1 overflow-hidden rounded-full bg-slate-200 lg:flex dark:bg-slate-700">
            <div
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            {/* Percentage */}
            <span
              className={`text-xs font-bold tabular-nums ${isDark ? 'text-purple-400' : 'text-purple-600'}`}
            >
              {Math.round(progress)}%
            </span>
            ...
            <AnimatePresence>
              {isExpanded && hasStats && (
                <motion.div
                  ...
                  className="hidden items-center gap-1.5 overflow-hidden lg:flex"
                >
            ...
            {hasStats && (
              ...
              >
                ...
                <span className="hidden lg:block">
                  {isExpanded ? (
                    <ChevronLeft
                      className={`h-3 w-3 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}
                    />
                  ) : (
                    <ChevronRight
                      className={`h-3 w-3 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}
            ...
            <div className={cn('mx-1 h-4 w-px', isDark ? 'bg-slate-700' : 'bg-slate-200')} />
            {/* Generation Code / Course ID */}
            {(generationCode || courseId) && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                ...
                <TooltipContent
                  side="top"
                  className="flex flex-col items-center gap-1 border border-slate-700 bg-slate-900 px-2 py-1.5 shadow-xl dark:bg-slate-800"
                  container={portalContainerRef.current}
                >
                  <span className="text-[10px] text-slate-400">
                    {generationCode ? 'Код генерации' : 'ID курса'}
                  </span>
        ...
        <div className="px-3 pb-1.5 lg:hidden">
          <div
            className={cn(
        ...
        <AnimatePresence>
          {isExpanded && hasStats && (
            ...
            >
              <div
                className={cn(
                  'grid grid-cols-3 gap-1 px-3 pb-1.5 sm:grid-cols-5',
                  isDark ? 'text-slate-100' : 'text-slate-900'
                )}
              >
                <MobileStat
                  ...
                  subValue={
                    generationProgress?.document_size
                      ? `${(generationProgress.document_size / 1000).toFixed(1)}KB`
                      : undefined
                  }
                ...
                <MobileStat
                  ...
                  subValue={
                    status === 'generating_structure' ||
                    (typeof status === 'string' && status.includes('stage_4_analyzing'))
                      ? t('stats.analyzing')
                      : undefined
                  }
