...
export default function LessonContent({ lesson, section, assets, lessonContent, enrichments }: LessonContentProps) {
  ...
  const { introText, mainContent } = useMemo(() => {
    ...
    if (lessonContent?.content) {
      ...
      if (innerContent) {
        const intro = typeof innerContent.intro === 'string' && innerContent.intro.trim()
          ? innerContent.intro
          : ''
  ...
  return (
    ...
    >
      ...
      {introText && (
        <div className="mb-8">
          <div className="p-6 bg-gradient-to-br from-blue-50/50 via-indigo-50/30 to-purple-50/50 dark:from-blue-900/10 dark:via-indigo-900/10 dark:to-purple-900/10 rounded-xl border border-blue-200/50 dark:border-blue-800/30 shadow-sm">
