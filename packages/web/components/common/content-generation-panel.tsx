...
export default function ContentGenerationPanel({
  ...
  const lessonsBySection = sections.reduce(
    (acc, section) => {
  ...
  const getSelectedLessons = (): Lesson[] => {
    switch (generationScope) {
      case 'all':
        return lessons
      case 'section':
        const sectionLessons = selectedSections.flatMap(
          (sectionId) => lessonsBySection[sectionId] || []
        )
    ...
    for (const lesson of lessonsToGenerate) {
      for (const formatId of selectedFormats) {
        ...
        setGenerationStatus((prev) =>
          prev.map((s) =>
            s.lessonId === lesson.id && s.formatId === formatId ? { ...s, status: 'generating' } : s
          )
        )
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="flex max-h-[90vh] max-w-4xl flex-col overflow-hidden border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
        <DialogHeader>
        ...
        <Tabs defaultValue="formats" className="flex flex-1 flex-col overflow-hidden">
          <TabsList className="grid w-full grid-cols-3">
            ...
            <TabsTrigger value="status" disabled={generationStatus.length === 0}>
              ...
              {generationStatus.length > 0 &&
                `(${generationStatus.filter((s) => s.status === 'completed').length}/${generationStatus.length})`}
            </TabsTrigger>
          </TabsList>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                {contentFormats.map((format) => {
                  ...
                  return (
                    <Card
                      key={format.id}
                      className={`cursor-pointer border border-gray-200 bg-white transition-all dark:border-gray-700 dark:bg-gray-800 ${
                    ...
                    >
                      <CardHeader className="pb-3">
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-3">
                            <div
                              className={`rounded-lg bg-gray-100 p-2 dark:bg-gray-700 ${format.color}`}
                            >
                            ...
                            <div>
                              ...
                              <Badge
                                variant="secondary"
                                className="mt-1 bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300"
                              >
                      ...
                      <CardContent>
                        ...
                        {!format.available && (
                          <Badge
                            variant="outline"
                            className="mt-2 border-gray-300 text-gray-600 dark:border-gray-600 dark:text-gray-400"
                          >
                            В разработке
                          </Badge>
                        )}
          ...
          <TabsContent value="lessons" className="flex flex-1 flex-col overflow-hidden">
              ...
              <div className="space-y-3 p-4">
                {sections.map((section) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className="flex cursor-pointer items-center gap-3 bg-gray-50 p-3 transition-colors hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700"
                          ...
                          >
                            <div className="space-y-1 p-2">
                              {sectionLessons.map((lesson) => (
                                <div
                                  key={lesson.id}
                                  className="flex items-center gap-3 rounded p-2 transition-colors hover:bg-gray-100 dark:hover:bg-gray-700"
                                >
                                  <Checkbox
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <div className="text-xs text-gray-600 dark:text-gray-400">
                                      Урок {lesson.lesson_number} • {lesson.duration_minutes || 5}{' '}
                                      мин
                                    </div>
                                  </div>
                                </div>
              ...
              <div className="space-y-3">
                {sections.map((section) => {
                  ...
                  return (
                    ...
                    >
                      <div className="mb-3 font-medium text-gray-900 dark:text-white">
                        {section.title}
                      </div>
                      <div className="space-y-2">
                        {sectionLessons.map((lesson) => {
                          const lessonStatuses = generationStatus.filter(
                            ...
                            <div key={lesson.id} className="pl-4">
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                {lessonStatuses.map((status) => {
                                  const format = contentFormats.find(
                                    (f) => f.id === status.formatId
                                  )
                                  return (
                                    <div
                                      key={`${status.lessonId}-${status.formatId}`}
