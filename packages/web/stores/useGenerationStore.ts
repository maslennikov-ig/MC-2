...
import { NodeStatus, TraceAttempt, DocumentStageData } from '@megacampus/shared-types'
...
const PHASE_TO_STEP_INDEX: Record<string, number> = {
  ...
  complete: 6,
  finish: 6,
  skip: 6, // Deduplicated documents skip to completion
  deduplicated: 6, // Alternative phase name for deduplication
}
  ...
  immer((set, get) => ({
      ...
      set((state) => {
        docs.forEach((doc) => {
          ...
          const completedSteps =
            doc.status === 'completed' ? TOTAL_DOCUMENT_STEPS - 1 : doc.status === 'active' ? 0 : -1
      ...
      set((state) => {
        ...
        switch (trace.stage) {
          ...
          case 'stage_2': {
            ...
            // - error_data = error
            // - init/start/complete/finish/skip/deduplicated phases = completed (these are markers, not processing steps)
            ...
            if (existing) {
              ...
              const newDoc = {
                ...
                totalSteps: TOTAL_DOCUMENT_STEPS,
                currentPhase: phase,
                priority: trace.input_data?.priority as
                  | 'CORE'
                  | 'IMPORTANT'
                  | 'SUPPLEMENTARY'
                  | undefined,
          ...
          case 'stage_6': {
            ...
            if (existing) {
              ...
              if (trace.error_data) {
                ...
                if (courseModule) {
                  ...
                  courseModule.completedLessons = moduleLessons.filter(
            ...
            if (stage6) {
              ...
              if (allLessons.some((l) => l.status === 'error')) {
                ...
                allLessons.length > 0 &&
                allLessons.every((l) => l.status === 'completed')
              ) {
        ...
        for (const trace of sorted) {
          ...
          switch (trace.stage) {
            ...
            case 'stage_2': {
              ...
              const docName = extractDocumentName(trace)
              // Marker phases (init, start, complete, finish, skip, deduplicated) are always 'completed' - they don't have output_data
              const isMarkerPhase =
                phase === 'init' ||
                phase === 'start' ||
                phase === 'complete' ||
    ...
    setModuleStructure: (modulesData) => {
      set((state) => {
        ...
        logger.devLog(
          '[GenerationStore] Set module structure:',
          state.modules.size,
          'modules,',
          state.lessons.size,
          'lessons'
