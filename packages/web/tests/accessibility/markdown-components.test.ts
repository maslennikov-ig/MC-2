...
test.describe('Markdown Component Accessibility', () => {
  ...
  test('Should inject test markdown content', async ({ page }) => {
    ...
    await page.evaluate((markdown) => {
      ...
      skipLink.className =
        'sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-[100] focus:bg-background focus:text-foreground focus:px-4 focus:py-2 focus:rounded-md focus:border focus:border-border focus:shadow-lg focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2'
  ...
  return \`Hello, \${name}!\`;
      ...
      ;['note', 'warning', 'danger', 'tip'].forEach((type) => {
        ...
        callout.className =
          'callout my-6 rounded-lg border-l-4 p-4 border-' + colorMap[type] + '-500'
        callout.innerHTML =
          ...
          type.slice(1) +
          '</div>' +
          '<div class="text-sm opacity-90">This is a ' +
          type +
          ' callout message.</div>' +
          '</div>' +
          '</div>'
      ...
      tableWrapper.className =
        'responsive-table-wrapper my-6 overflow-x-auto scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent'
