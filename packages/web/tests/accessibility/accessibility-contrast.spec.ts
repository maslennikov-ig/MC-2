...
test.describe('WCAG AA Contrast Compliance - Light Theme', () => {
  ...
  test('should meet WCAG AA contrast requirements for light theme', async ({ page }) => {
    ...
    const contrastViolations = accessibilityScanResults.violations.filter(
      (violation) => violation.id.includes('contrast') || violation.id === 'color-contrast'
    )
  ...
  test('should generate accessibility report', async ({ page }) => {
    ...
    const report = {
      ...
      contrastIssues: accessibilityScanResults.violations.filter(
        (v) => v.id.includes('contrast') || v.id === 'color-contrast'
      ).length,
      criticalViolations: accessibilityScanResults.violations
