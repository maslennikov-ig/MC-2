...
import { mockEnrichmentApi } from './fixtures/enrichment-fixtures'
...
test.describe('Enrichment Inspector Accessibility', () => {
  ...
  test('drag handles should have accessible names', async ({ page }) => {
    ...
    const isFocusable =
      tagName === 'button' || tabIndex !== '-1' || role === 'button' || role === 'application'
  ...
  test('discovery cards should be keyboard accessible', async ({ page }) => {
    ...
    for (let i = 0; i < cardCount; i++) {
      ...
      const isInteractive = role === 'button' || tagName === 'button' || tabIndex === '0'
  ...
  test('status indicators should be readable by screen readers', async ({ page }) => {
    ...
    if (statusCount === 0) {
      ...
      for (let i = 0; i < statusCount; i++) {
        ...
        console.log(
          `Status indicator ${i}: role=${role}, aria-label=${ariaLabel}, aria-live=${ariaLive}`
        )
      }
    }
    ...
    if ((await generatingItems.count()) > 0) {
      ...
      const hasLoadingIndicator =
        ariaBusy === 'true' || (await progressBar.count()) > 0 || (await spinner.count()) > 0
  ...
  test('error states should be announced to screen readers', async ({ page }) => {
    ...
    if ((await failedItems.count()) > 0) {
      ...
      const hasErrorText =
        itemText.toLowerCase().includes('error') || itemText.toLowerCase().includes('failed')
