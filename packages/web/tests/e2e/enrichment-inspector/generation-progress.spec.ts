...
test.describe('Enrichment Inspector Generation Progress', () => {
  ...
  test('should show animated dots during generation', async ({ page }) => {
    ...
    if (hasAnimation) {
      ...
      const animatedDots = page.locator('[class*="dots"], [class*="ellipsis"], .animate-pulse')
      ...
      console.log(
        `Animation check: ${JSON.stringify(generatingItemStyles)}, nested: ${hasNestedAnimation}`
      )
    }
  })
