...
test.describe('Enrichment Inspector i18n - English Locale', () => {
  ...
  test('should display English status texts', async ({ page }) => {
    ...
    const possibleStatuses = [EN_TEXTS.pending, EN_TEXTS.generating, EN_TEXTS.completed]

    // At least one status should be visible
    const hasEnglishStatus = possibleStatuses.some((status) =>
      pageText.toLowerCase().includes(status.toLowerCase())
    )
  ...
  test('should display English action buttons', async ({ page }) => {
    ...
    const hasEnglishBack =
      ...
      (backButtonAriaLabel &&
        backButtonAriaLabel.toLowerCase().includes(EN_TEXTS.back.toLowerCase()))
...
test.describe('Enrichment Inspector i18n - Russian Locale', () => {
  ...
  test('should display Russian status texts', async ({ page }) => {
    ...
    const possibleStatuses = [RU_TEXTS.pending, RU_TEXTS.generating, RU_TEXTS.completed]
