...
test.describe('Enrichment Inspector Deep Links', () => {
  ...
  test('should open inspector from asset dock', async ({ page }) => {
    ...
    if ((await assetDock.count()) > 0) {
      ...
      await page.goto(
        '/en/courses/generating/test-course?panel=enrichments&lesson=lesson-with-enrichments'
      )
      await page.waitForLoadState('networkidle')
  ...
  test('should navigate to specific enrichment detail via deep link', async ({ page }) => {
    ...
    const deepLinkUrl =
      '/en/courses/generating/test-course?panel=enrichments&lesson=lesson-with-enrichments&enrichment=e1'
    ...
    if ((await inspectorPage.panel.count()) > 0) {
      ...
      if (isDetailVisible) {
        ...
        const selectedItem = page.locator(
          '[data-enrichment-id="e1"][aria-selected="true"], [data-enrichment-id="e1"].selected'
        )
  ...
  test('should sync inspector state with selected lesson', async ({ page }) => {
    ...
    if ((await inspectorPage.panel.count()) > 0) {
      ...
      const selectedNode = page.locator(
        '[data-node-id].selected, [data-node-id][aria-selected="true"]'
      )
  ...
  test('should preserve deep link state on page refresh', async ({ page }) => {
    ...
    console.log(
      `State preserved: before=${beforeRefreshPanelVisible}, after=${afterRefreshPanelVisible}`
    )
  })
  ...
  test('should handle invalid deep link gracefully', async ({ page }) => {
    ...
    const invalidUrl =
      '/en/courses/generating/test-course?panel=enrichments&lesson=lesson-with-enrichments&enrichment=invalid-id'
    ...
    const handledGracefully = (await hasError.count()) > 0 || hasList || hasEmptyState || !hasPanel // Or simply not open the panel for invalid links
  ...
  test('should handle invalid lesson ID in deep link', async ({ page }) => {
    ...
    const isPageFunctional =
      (await notFound.count()) > 0 || (await page.locator('body').count()) > 0
