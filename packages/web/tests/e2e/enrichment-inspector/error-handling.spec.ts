...
test.describe('Enrichment Inspector - Error Handling', () => {
  ...
  test('should show error fallback when component crashes', async ({ page }) => {
    ...
    await mockApiError(page, '**/api/trpc/enrichment.list*', 'Internal server error')
  ...
  test('should show error state for failed enrichments', async ({ page }) => {
    ...
    const lesson = TEST_COURSES[COURSE_SLUG].lessons.find((l) => l.id === LESSON_WITH_FAILED)
    ...
    await page.route('**/api/trpc/enrichment.list*', async (route) => {
      ...
      if (requestCount === 1) {
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({
            result: {
              data: [{ id: 'e1', type: 'quiz', status: 'completed', display_order: 1 }],
            },
          }),
        })
      }
  ...
  test('should handle concurrent errors gracefully', async ({ page }) => {
    ...
    const hasErrorState = await page
      .locator('[role="alert"], [class*="error"], text=/error/i')
      .first()
      .isVisible()
      .catch(() => false)
