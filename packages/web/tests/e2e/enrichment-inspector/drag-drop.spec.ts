...
import { TEST_COURSES, mockEnrichmentApi, waitForAnimation } from './fixtures/enrichment-fixtures'
...
test.describe('Enrichment Inspector - Drag and Drop', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/trpc/enrichment.list*', async (route) => {
      const lesson = TEST_COURSES[COURSE_SLUG].lessons.find((l) => l.id === LESSON_WITH_ENRICHMENTS)
      await route.fulfill({
        status: 200,
  ...
  test('should show visual feedback during drag', async ({ page }) => {
    ...
    const hoverCursor = await firstHandle.evaluate((el) => window.getComputedStyle(el).cursor)

    // Cursor should indicate draggability
    const isDraggableCursor = ['grab', 'move', 'pointer', '-webkit-grab'].includes(hoverCursor)
