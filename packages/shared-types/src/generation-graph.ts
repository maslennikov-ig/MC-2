...
// `StaticGraphData` names the styling contract that now lives in the layout module.
import type { NodeStyles } from './generation-graph-layout';
...
// ElkJS structures, `LAYOUT_OPTIONS`, `NodeColorScheme`, `NodeStyles`,
// `NODE_STYLES` and the two type registries live next door; re-exported here so
// that every existing import path keeps working.
export * from './generation-graph-layout';
