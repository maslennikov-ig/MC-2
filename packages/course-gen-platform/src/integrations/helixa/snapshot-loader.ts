...
import type {
  CourseFromRoleGuideRelation,
  GenerationOriginCommandV1,
  JsonValue,
  KnowledgeObjectKind,
} from './contract';
...
function fileSource(
  ...
  return {
    ...
    version: file.approvedVersion ?? file.hash,
...
export function bindAcceptedCourseSources<T extends { id: string; hash: string }>(
  manifestInput: unknown,
  files: T[]
): Array<T & { approved: true; approvedVersion: string }> {
  ...
  return manifest.map((source: { document_id: string; source_version_hash: string }) => {
    ...
    if (!file || file.hash !== source.source_version_hash)
      throw new KnowledgeSyncPreparationError('provenance', false);
...
export async function mapCompletedCourse(input: {
  ...
  const { course } = input;
  if (course.generation_status !== 'completed' || !course.generation_completed_at)
    throw new KnowledgeSyncPreparationError('event_identity', false);
  if (
    input.files.some(
      file =>
        file.organization_id !== course.organization_id ||
        file.course_id !== course.id ||
        !file.approved ||
        file.approvedVersion !== file.hash
    )
  )
    throw new KnowledgeSyncPreparationError('provenance', false);
  const originCommand = generationOrigin(input.generationOrigin, {
    kind: 'COURSE',
    id: course.id,
    organizationId: course.organization_id,
  });
  const relation = courseFromRoleGuideRelation(
    input.jobInstructionSource,
    input.generationOrigin,
    course
  );
  return {
    kind: 'COURSE',
    id: course.id,
    organizationId: course.organization_id,
    completedAt: course.generation_completed_at,
    title: course.title,
    ...
    structure: jsonObject(course.course_structure),
    blocks: [],
    lessons: input.lessonContents
    ...
    sources: input.files.map(file =>
      fileSource(file, 'COURSE', course.id, 'file_catalog', input.readBytes)
    ),
...
// eslint-disable-next-line @typescript-eslint/require-await
export async function mapCompletedRoleGuide(input: {
  playbook: CompletedRoleGuideRow;
  sources: RoleGuideSourceRow[];
  readBytes: ReadBytes;
  generationOrigin?: GenerationOriginRow | null;
}): Promise<KnowledgeExportSnapshot> {
  const { playbook } = input;
  if (playbook.status !== 'completed' || !playbook.completed_at || !playbook.final_markdown?.trim())
    throw new KnowledgeSyncPreparationError('event_identity', false);
  const originCommand = generationOrigin(input.generationOrigin, {
    kind: 'ROLE_GUIDE',
    id: playbook.id,
  ...
  for (const source of input.sources) {
    if (
      source.status !== 'ready' ||
      source.organization_id !== playbook.organization_id ||
      source.playbook_id !== playbook.id
    )
      throw new KnowledgeSyncPreparationError('provenance', false);
    if (source.source_type === 'file') {
      if (
        !source.file ||
        source.file.organization_id !== playbook.organization_id ||
        source.file.course_id !== null
      )
        throw new KnowledgeSyncPreparationError('provenance', false);
      sources.push(
        fileSource(
          { ...source.file, approved: true },
          'ROLE_GUIDE',
          playbook.id,
          'career_playbook_source',
          input.readBytes,
          source.id
        )
      );
      ...
      sources.push({
        ...
        fileName: source.filename ?? `source-${source.id}.txt`,
        mediaType: 'text/plain',
        readOriginalBytes: () => Promise.resolve(bytes),
        trustedMarkdown: source.text,
      });
    }
  }
  ...
  return {
    ...
    title: playbook.position_title?.trim() || 'Role Guide',
    ...
    blocks: blockEntries.map(([key, value]) => ({ key, value })),
    lessons: [],
    sources,
    ...(originCommand ? { originCommand } : {}),
  };
