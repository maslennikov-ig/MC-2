...
export interface KnowledgeSyncOutboxEntry {
  id: string;
  eventId: string;
...
function safeError(failure: DeliveryFailure): string {
  return failure.kind === 'http' ? `HTTP ${failure.status}` : 'Transient delivery failure';
}
...
export async function processKnowledgeSyncOutboxEntry(input: {
  ...
  try {
    ...
    await deliverClaimedKnowledgeSync(
      { id: input.entry.id, eventId: input.entry.eventId, rawBody },
      input.delivery
    );
    return (await input.repository.markDelivered(input.entry.id, input.entry.leaseToken))
      ? 'delivered'
      : 'lost_lease';
  } catch (error) {
    if (error instanceof KnowledgeSyncPreparationError) {
      if (!error.retryable || input.entry.attempts >= KNOWLEDGE_SYNC_MAX_ATTEMPTS) {
        return (await input.repository.markTerminal(
          input.entry.id,
          input.entry.leaseToken,
      ...
      const next = new Date(
        (input.now ?? new Date()).getTime() +
          computeRetryDelayMs(input.entry.attempts, input.entry.eventId)
      );
      return (await input.repository.reschedule(
        input.entry.id,
        input.entry.leaseToken,
        next,
    ...
    const failure: DeliveryFailure =
      candidate.kind === 'http' && typeof candidate.status === 'number'
    ...
    return (await input.repository.markTerminal(
      input.entry.id,
      input.entry.leaseToken,
      failure.kind === 'http' ? `HTTP ${failure.status}` : 'Retry budget exhausted'
    ))
      ? 'terminal'
      : 'lost_lease';
  }
