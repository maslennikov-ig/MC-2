...
import { mapCompletedCourse, mapCompletedRoleGuide } from './snapshot-loader';
...
type OutboxRow = {
  ...
  completed_at: string; raw_body_base64: string | null;
};

export async function claimKnowledgeSyncOutbox(batchSize = 10): Promise<KnowledgeSyncOutboxEntry[]> {
  const rows = expectData(await client().rpc<OutboxRow[]>('claim_helixa_knowledge_sync_outbox', { p_batch_size: batchSize }), 'Failed to claim Helixa knowledge outbox');
  return rows.map(row => ({ id: row.id, eventId: row.event_id, objectKind: row.object_kind, objectId: row.object_id, organizationId: row.organization_id, completedAt: row.completed_at, rawBody: row.raw_body_base64 ? Buffer.from(row.raw_body_base64, 'base64') : null }));
}

export function createKnowledgeSyncOutboxRepository(): KnowledgeSyncOutboxRepository {
  return {
    async persistRawBodyOnce(id, rawBody, payloadHash) {
      const base64 = expectData(await client().rpc<string>('freeze_helixa_knowledge_sync_payload', { p_id: id, p_raw_body_utf8: rawBody.toString('utf8'), p_payload_hash: payloadHash }), 'Failed to freeze Helixa payload');
      return Buffer.from(base64, 'base64');
    },
    async markDelivered(id) {
      const result = await client().from('helixa_knowledge_sync_outbox').update({ status: 'delivered', delivered_at: new Date().toISOString(), last_error: null }).eq('id', id);
      if (result.error) throw new Error(`Failed to mark Helixa delivery: ${result.error.message}`);
    },
    async reschedule(id, nextAttemptAt, error) {
      const result = await client().from('helixa_knowledge_sync_outbox').update({ status: 'retryable', next_attempt_at: nextAttemptAt.toISOString(), last_error: error }).eq('id', id);
      if (result.error) throw new Error(`Failed to reschedule Helixa delivery: ${result.error.message}`);
    },
    async markTerminal(id, error) {
      const result = await client().from('helixa_knowledge_sync_outbox').update({ status: 'terminal', last_error: error }).eq('id', id);
      if (result.error) throw new Error(`Failed to record Helixa refusal: ${result.error.message}`);
    },
  };
}
...
export async function loadKnowledgeSnapshot(entry: Pick<KnowledgeSyncOutboxEntry, 'objectKind' | 'objectId' | 'organizationId' | 'completedAt'>) {
  ...
  if (entry.objectKind === 'COURSE') {
    ...
    const acceptedRunResult = await db.from<{ source_manifest: Array<{ document_id: string }> }>('document_evidence_runs').select('source_manifest').eq('course_id', entry.objectId).eq('organization_id', entry.organizationId).eq('status', 'accepted').order('completed_at', { ascending: false }).limit(1).maybeSingle();
    if (acceptedRunResult.error) throw new Error(`Failed to load accepted Course provenance: ${acceptedRunResult.error.message}`);
    const sourceIds = acceptedRunResult.data?.source_manifest.map(item => item.document_id) ?? [];
    ...
    return mapCompletedCourse({ course: course as never, lessonContents: lessons as never[], files: (files as Array<Record<string, unknown>>).map(file => ({ ...file, approved: true })) as never[], readBytes });
  }
...
export function createSupabaseReconcileRepository(): ReconcileRepository {
  return {
    async listCompleted() {
      const db = client();
      const courses = expectData(await db.from<Array<{ id: string; organization_id: string; generation_completed_at: string }>>('courses').select('id, organization_id, generation_completed_at').eq('generation_status', 'completed'), 'Failed to list completed Courses');
      const guides = expectData(await db.from<Array<{ id: string; organization_id: string; completed_at: string }>>('career_playbooks').select('id, organization_id, completed_at').eq('status', 'completed'), 'Failed to list completed Role Guides');
      return [
...
export function readKnowledgeSyncRuntimeConfig(environment: NodeJS.ProcessEnv = process.env): KnowledgeSyncRuntimeConfig {
  ...
  if (!endpoint || !hmacKey || !externalSystemId) throw new Error('Helixa knowledge sync configuration is incomplete');
  return { endpoint, hmacKey, externalSystemId, environment: environment.APP_ENV ?? environment.NODE_ENV ?? 'development', externalProjectId: environment.HELIXA_DESTINATION_PROJECT_ID ?? null };
}
