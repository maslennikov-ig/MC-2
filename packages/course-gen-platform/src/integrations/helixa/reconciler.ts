...
export interface ReconcileRepository {
  listCompleted(): Promise<CompletedObject[]>;
  insertMissing(intents: Array<CompletedObject & { eventId: string }>): Promise<number>;
}

export async function reconcileCompletedKnowledgeObjects(
  ...
  if (options.apply !== true)
    return { dryRun: true, discovered: intents.length, inserted: 0, intents };
  return {
    dryRun: false,
    discovered: intents.length,
    inserted: await repository.insertMissing(intents),
    intents,
  };
}
