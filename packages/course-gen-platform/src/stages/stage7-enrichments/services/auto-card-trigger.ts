...
async function triggerLessonEnrichment(
  ...
  try {
    ...
    // Create enrichment record with insert (race conditions handled via unique constraint)
    // Note: We use insert instead of upsert because the unique index is partial
    // (WHERE title IS DISTINCT FROM 'course-card') and PostgreSQL doesn't support
    // ON CONFLICT with partial indexes without explicit constraint name
    ...
    if (insertError) {
      // 23505 = unique_violation - record already exists (race condition)
