...
export function inferContentArchetype(section: SectionBreakdown): ContentArchetype {
  ...
  if (!section) {
    logger.warn(
      { section },
      'inferContentArchetype: received null/undefined section, defaulting to concept_explainer'
    );
  ...
  if (
    pedagogicalApproach.toLowerCase().includes('case study') ||
...
export function inferHookStrategy(
  ...
  const statisticPatterns =
    /\d+%|\d+\s*percent|statistic|data|metric|measure|number|rate|ratio|percentage/i;
