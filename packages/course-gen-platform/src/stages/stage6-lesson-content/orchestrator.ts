...
function buildJudgeProgressSummary(
  ...
  switch (recommendation) {
    ...
    case 'ESCALATE_TO_HUMAN':
      outcome = isRussian ? `→ Передано на проверку человеку` : `→ Escalated to human review`;
      break;
    default:
      outcome = isRussian ? `→ Оценка завершена` : `→ Evaluation completed`;
  }
  ...
  const isCompleted =
    recommendation === 'ACCEPT' || recommendation === 'ACCEPT_WITH_MINOR_REVISION';
 ...
 */
async function sectionRegeneratorNode(
  state: LessonGraphStateType
