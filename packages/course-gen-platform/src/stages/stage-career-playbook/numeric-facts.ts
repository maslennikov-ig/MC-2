...
function classifyNumericFact(input: {
  ...
  const evidence = normalizeForSearch(input.evidenceText);
  const raw = normalizeForSearch(input.rawText);
  ...
  if (evidence && evidence.includes(raw)) {
    const isBenchmark =
      evidence.includes('benchmark') ||
      evidence.includes('бенчмарк') ||
      evidence.includes('industry') ||
      evidence.includes('рынок');
    if (isBenchmark) {
      return {
        status: 'benchmark',
