...
function getInitialStateForJobType(jobType: JobType): string {
  switch (jobType) {
    ...
    default:
      ...
      logger.warn(
        { jobType },
        'Unknown job type for FSM initialization, defaulting to stage_2_init'
      );
      return 'stage_2_init';
  }
}
  ...
  try {
    ...
    queueEvents.on('added', ({ jobId, name }) => {
      void (async () => {
        ...
        // Support both camelCase and snake_case field names (some jobs use snake_case internally)
        const jobData = job.data as Record<string, unknown>;
        const courseId = (jobData.courseId || jobData.course_id) as string | undefined;
        const userId = (jobData.userId || jobData.user_id) as string | undefined;
        const organizationId = (jobData.organizationId || jobData.organization_id) as
          | string
          | undefined;
