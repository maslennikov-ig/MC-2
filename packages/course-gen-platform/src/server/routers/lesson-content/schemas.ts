...
export const startStage6InputSchema = z.object({
  ...
  lessonSpecs: z
    .array(LessonSpecificationV2Schema)
    .min(1, 'At least one lesson specification required'),
 ...
 */
export const partialGenerateInputSchema = z
  .object({
    /** Course ID to generate lessons for */
    courseId: z.string().uuid('Invalid course ID'),
    ...
    lessonIds: z
      .array(z.string().regex(/^\d+\.\d+$/, 'Lesson ID must be in format "section.lesson"'))
      .optional(),
  ...
  .refine(
    data =>
      ...
      (data.sectionIds && data.sectionIds.length > 0),
