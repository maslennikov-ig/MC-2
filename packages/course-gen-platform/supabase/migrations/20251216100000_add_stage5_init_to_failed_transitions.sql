-- ============================================================================
-- Migration: add_stage5_init_to_failed_transitions
-- Purpose: Allow retry from failed state to stage_5_init (and other missing stages)
-- Date: 2025-12-16
-- Author: Claude Code
--
-- Problem: When Stage 5 fails and BullMQ retries, the handler tries to set
-- status to stage_5_init, but FSM doesn't allow failed -> stage_5_init.
--
-- Solution: Add all stage_X_init transitions from failed/cancelled/completed
-- ============================================================================
...
BEGIN
  ...
  --
  -- Terminal states (failed, cancelled, completed) can restart to ANY stage init
  -- ==========================================================================
