-- Migration: 20260316120000_add_failed_at_stage_to_rpc.sql
-- Purpose: Add failed_at_stage tracking to update_course_progress RPC function
-- When a step fails, record which stage (p_step_id) failed so the UI can
-- display the error on the correct pipeline graph node.
-- Date: 2026-03-16
--
-- Change: Added one line to the UPDATE statement:
--   failed_at_stage = CASE WHEN p_status = 'failed' THEN p_step_id ELSE failed_at_stage END,
--
-- The failed_at_stage column was added in 20251207200000_add_failed_at_stage.sql
-- but was never populated by this RPC function until now.

-- Must DROP first because PostgreSQL cannot alter parameter defaults via CREATE OR REPLACE
DROP FUNCTION IF EXISTS update_course_progress(UUID, INTEGER, TEXT, TEXT, TEXT, JSONB, JSONB);

CREATE FUNCTION update_course_progress(
  p_course_id UUID,
  p_step_id INTEGER,
  p_status TEXT,
  p_message TEXT,
...
BEGIN
  ...
  SET
    ...
    -- Track which stage failed (for UI graph node highlighting)
    failed_at_stage = CASE WHEN p_status = 'failed' THEN p_step_id ELSE failed_at_stage END,
    -- Update timestamps
...
COMMENT ON FUNCTION update_course_progress(UUID, INTEGER, TEXT, TEXT, TEXT, JSONB, JSONB) IS 'Update course generation progress (JSONB) and generation_status (FSM ENUM) atomically. Updated 2026-03-16 to set failed_at_stage on failure.';

-- Ensure proper access grants (fully qualified to avoid ambiguity with overloaded function)
GRANT EXECUTE ON FUNCTION update_course_progress(UUID, INTEGER, TEXT, TEXT, TEXT, JSONB, JSONB) TO service_role;
GRANT EXECUTE ON FUNCTION update_course_progress(UUID, INTEGER, TEXT, TEXT, TEXT, JSONB, JSONB) TO authenticated;
