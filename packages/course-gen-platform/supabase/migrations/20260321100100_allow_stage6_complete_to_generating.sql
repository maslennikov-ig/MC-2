-- Fix FSM race: allow stage_6_complete -> stage_6_generating transition
-- Bug: partial re-generation (partialGenerate, generateMissing) needs to transition
-- back to stage_6_generating from stage_6_complete, but FSM trigger blocks it.
-- Caused: 16 errors "Invalid generation status transition: stage_6_complete -> stage_6_generating"
...
BEGIN
  -- Only validate when generation_status actually changes
  IF OLD.generation_status IS NULL OR OLD.generation_status = NEW.generation_status THEN
    RETURN NEW;
  END IF;

  v_valid_transitions := '{
    "pending": ["stage_2_init", "cancelled"],
    "stage_2_init": ["stage_2_processing", "stage_2_complete", "failed", "cancelled"],
    ...
    "stage_2_awaiting_approval": ["stage_2_complete", "stage_3_init", "cancelled"],
    "stage_3_init": ["stage_3_generating", "stage_3_complete", "failed", "cancelled"],
    "stage_3_generating": ["stage_3_complete", "stage_3_awaiting_approval", "failed", "cancelled"],
    "stage_3_complete": ["stage_3_awaiting_approval", "stage_4_init", "failed", "cancelled"],
    "stage_3_awaiting_approval": ["stage_3_complete", "stage_4_init", "finalizing", "cancelled"],
    "stage_4_init": ["stage_4_generating", "stage_4_complete", "stage_4_awaiting_approval", "failed", "cancelled"],
    "stage_4_generating": ["stage_4_complete", "stage_4_awaiting_approval", "failed", "cancelled"],
    "stage_4_complete": ["stage_4_awaiting_approval", "stage_5_init", "finalizing", "failed", "cancelled"],
    "stage_4_awaiting_approval": ["stage_4_complete", "stage_5_init", "finalizing", "cancelled"],
