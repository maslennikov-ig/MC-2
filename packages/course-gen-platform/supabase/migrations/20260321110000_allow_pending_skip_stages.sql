-- Fix: allow pending → stage_3_init and stage_4_init transitions
-- Bug: generation.initiate has 3 paths:
--   Path 1: pending → stage_2_init (files need processing) - works
--   Path 2: pending → stage_3_init (files already indexed) - BLOCKED
--   Path 3: pending → stage_4_init (no files) - BLOCKED
-- These are legitimate flows when documents are pre-processed.
