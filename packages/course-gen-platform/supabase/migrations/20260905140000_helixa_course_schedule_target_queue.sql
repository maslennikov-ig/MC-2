-- `schedule_helixa_course_from_role_guide` wrote `target_queue = 'course-generation'` as a
-- literal. The outbox processor only claims rows whose `target_queue` equals its own
-- `QUEUE_NAME`, and dev runs with `BULLMQ_QUEUE_NAME=course-generation-dev`. So on dev the
-- row this function inserted was never claimed and the course it scheduled never started.
-- Nothing caught it because no Helixa command has ever run against a database.
--
-- The queue name now comes from the caller, which reads the same `QUEUE_NAME` the FSM
-- initialization handler passes to `initialize_fsm_with_outbox`. An empty value is refused
-- rather than defaulted, because a default here is exactly how the literal survived.
--
-- DROP then CREATE, not CREATE OR REPLACE: the parameter list changes, and two overloads
-- sharing a name make every PostgREST `rpc()` call to it unresolvable. One name, one
-- signature. Everything else in the body is unchanged from the 2026-09-05 definition.
...
REVOKE ALL ON FUNCTION schedule_helixa_course_from_role_guide(TEXT, TEXT, UUID, UUID, UUID, JSONB, JSONB, UUID, INTEGER, TEXT) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION schedule_helixa_course_from_role_guide(TEXT, TEXT, UUID, UUID, UUID, JSONB, JSONB, UUID, INTEGER, TEXT) TO service_role;
