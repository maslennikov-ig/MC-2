...
describe('Qdrant Vector Database Integration Tests', () => {
  ...
  describe.skipIf(!qdrantAvailable)(
    'Scenario 1: Collection created with HNSW configuration',
    () => {
  ...
  describe.skipIf(!qdrantAvailable)('Scenario 2: Test vectors uploaded successfully', () => {
    ...
    it('should upload vectors in batches', async () => {
      ...
      console.log(
        `✓ Uploaded ${uploadResult.points_uploaded} vectors in ${uploadResult.batch_count} batches`
      );
  ...
  describe.skipIf(!qdrantAvailable)(
    'Scenario 3: Semantic search returns top-K results with <30ms p95 latency',
    () => {
      beforeAll(async () => {
      ...
      it('should apply score threshold filtering', async () => {
        ...
        console.log(
          `✓ Score threshold filtering applied (${searchResults.length} results above 0.9)`
        );
      });
    }
  );
  ...
  describe.skipIf(!qdrantAvailable)(
    'Scenario 4: Search with course_id filter returns only vectors for specified course',
    () => {
      beforeAll(async () => {
      ...
      it('should filter results by course_id', async () => {
        ...
        console.log(
          `✓ Course filter applied: ${searchResults.length} results from course ${TEST_COURSE_1}`
        );
      });
      ...
      it('should return different results for different courses', async () => {
        ...
        console.log(
          `✓ Course 1: ${course1Results.length} results, Course 2: ${course2Results.length} results`
        );
  ...
  describe.skipIf(!qdrantAvailable)(
    'Scenario 5: Multi-tenant isolation works (organization_id filtering)',
    () => {
      beforeAll(async () => {
      ...
      it('should enforce organization isolation in search', async () => {
        ...
        console.log(
          `✓ Organization filter enforced: ${org1Results.length} results from org ${TEST_ORG_1}`
        );
      });
      ...
      it('should support combined organization and course filtering', async () => {
        ...
        console.log(
          `✓ Combined filters applied: ${searchResults.length} results matching both org and course`
        );
      });
