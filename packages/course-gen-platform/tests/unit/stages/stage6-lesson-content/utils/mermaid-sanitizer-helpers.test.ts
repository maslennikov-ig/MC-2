...
describe('removeRawQuotes', () => {
  ...
  it('preserves valid quoted node labels', () => {
    const ctx = makeCtx('A["Секретарь (Senior)"] --> B');
    const result = removeRawQuotes(ctx);
    expect(result.sanitized).toBe('A["Секретарь (Senior)"] --> B');
    expect(result.modified).toBe(false);
  });
});
...
describe('processMermaidBlock', () => {
  ...
  it('keeps labels with parentheses quoted after the full sanitizer sequence', () => {
    const content = 'flowchart TD\n  A[Секретарь (Senior)] --> B{Выбор сценария}';
    const result = processMermaidBlock(content, 0);
    expect(result.sanitized).toContain('A["Секретарь (Senior)"]');
    expect(result.sanitized).not.toContain('A[Секретарь (Senior)]');
  });
});
