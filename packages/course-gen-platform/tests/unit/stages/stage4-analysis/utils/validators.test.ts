...
describe('formatErrorMessage', () => {
    ...
    it('returns errors.insufficient_scope for insufficient scope error', () => {
        expect(
            formatErrorMessage(new Error('Insufficient scope for minimum 10 lessons'))
        ).toBe('errors.insufficient_scope');
    });
...
describe('validateJobInput', () => {
    ...
    it('throws for invalid document_summaries (missing fields)', () => {
        const input = validInput({
            document_summaries: [
                { document_id: 'doc-1', file_name: 'file.pdf', processed_content: '' },
            ],
        });
...
describe('PROGRESS_MESSAGES', () => {
    ...
    it('all messages are non-empty strings', () => {
        for (const [key, value] of Object.entries(PROGRESS_MESSAGES)) {
...
describe('PROGRESS_RANGES', () => {
    it('starts at 0 and ends at 100', () => {
        const allStarts = Object.values(PROGRESS_RANGES).map(r => r.start);
        const allEnds = Object.values(PROGRESS_RANGES).map(r => r.end);
        expect(Math.min(...allStarts)).toBe(0);
        expect(Math.max(...allEnds)).toBe(100);
    });

    it('start is always less than end for each range', () => {
        for (const [key, range] of Object.entries(PROGRESS_RANGES)) {
