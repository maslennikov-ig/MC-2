...
import { reviseJSON } from '@/stages/stage4-analysis/utils/revision-chain';
...
describe('Revision Chain Service', () => {
  ...
  describe('Retry logic', () => {
    ...
    it('should succeed on last retry attempt', async () => {
      ...
      const result = await reviseJSON('Original prompt', '{"a":', 'Unexpected end', mockModel, 3);
    ...
    it('should handle Phase 2 output with Russian content', async () => {
      const repairedJSON = JSON.stringify({
        recommended_structure: {
          ...
          scope_reasoning: 'Курс по закупкам требует углубленного изучения законодательства',
          lesson_duration_minutes: 15,
