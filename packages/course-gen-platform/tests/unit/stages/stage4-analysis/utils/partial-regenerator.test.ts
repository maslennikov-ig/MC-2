...
import { regenerateFields } from '@/stages/stage4-analysis/utils/partial-regenerator';
import { z } from 'zod';
...
describe('Partial Regeneration Service', () => {
  ...
  describe('Nested structure regeneration', () => {
    it('should regenerate failed nested field', async () => {
      ...
      expect(metadata.regeneratedFields.some(f => f.includes('scope_reasoning'))).toBe(true);
    });
  ...
  describe('Error handling', () => {
    ...
    it('should throw if LLM API call fails', async () => {
      ...
      (mockModel.invoke as Mock).mockRejectedValueOnce(new Error('LLM API timeout'));
