...
function scheduledObserverRepository(claimGeneration: number) {
  return {
    claimScheduled: vi.fn(async () => ({
      ...
      commandId: courseCommand.commandId,
      ...
      payloadHash: courseCommand.payloadHash,
      proposalId: courseCommand.proposalId,
      approvedRevision: courseCommand.approvedRevision,
      objectKind: 'COURSE' as const,
      objectId: '44444444-4444-4444-8444-444444444444',
      status: 'executing' as const,
      acceptedAt: '2026-08-23T10:16:00.000Z',
      updatedAt: '2026-08-23T10:20:00.000Z',
      claimGeneration,
      leaseToken: '77777777-7777-4777-8777-777777777777',
    })),
    actionRequired: vi.fn(async () => true),
    reconcileCompleted: vi.fn(),
    failObserved: vi.fn(async () => true),
    completeObserved: vi.fn(async () => true),
    returnScheduled: vi.fn(async () => true),
    lookup: vi.fn(),
  };
}
...
describe('server-only Helixa generation commands', () => {
  it('canonicalizes keys by unsigned UTF-8 bytes and rejects non-safe-integer numbers', () => {
    ...
    const bmp = '\uE000';
    expect(canonicalGenerationJsonV1({ [supplementary]: 1, [bmp]: 2 })).toBe(
      `{"${bmp}":2,"${supplementary}":1}`
    );
  ...
  it('rejects identity injection, unknown fields, bad namespace, duplicates, and source order', () => {
    ...
    expect(() =>
      parseHelixaGenerationCommand({ ...jobCommand, commandId: courseCommand.commandId })
    ).toThrow();
    expect(() =>
      parseHelixaGenerationCommand({
        ...jobCommand,
  ...
  it('keeps migration additive, disabled, immutable, and separate from the legacy ledger', async () => {
    ...
    expect(sql).toContain(
      'course_from_job_instruction_creation_enabled BOOLEAN NOT NULL DEFAULT false'
    );
    ...
    expect(sql.match(/prevent_helixa_generation_proof_mutation/g)?.length).toBeGreaterThanOrEqual(
      3
    );
    expect(sql).not.toMatch(
      /DROP TABLE helixa_course_creation_commands|ALTER TABLE helixa_course_creation_commands/
    );
  ...
  it('closes binding scope in Postgres adapters and never accepts caller identity', async () => {
        ...
        data: [
          {
            ...
            destination_binding_id: binding.destinationBindingId,
        ...
        data: [
          {
            ...
            command_hash: generationCommandHash(parseHelixaGenerationCommand(jobCommand)),
            proposal_payload_hash: jobCommand.payloadHash,
            object_kind: 'ROLE_GUIDE',
            object_id: '33333333-3333-4333-8333-333333333333',
            status: 'reserved',
            accepted_at: null,
            updated_at: '2026-08-23T10:16:00.000Z',
            conflict: false,
            mutation_owner: true,
    ...
    const resolved = await createPostgresHelixaGenerationBindingAuthority(client).resolve(
      binding.bindingId
    );
  ...
  it('looks up a durable command from a fresh repository instance after restart', async () => {
      ...
      data: [
        {
          ...
          accepted_at: '2026-08-23T10:16:00.000Z',
          updated_at: '2026-08-23T10:20:00.000Z',
          lease_token: null,
          claim_generation: 2,
          command_kind: 'CREATE_JOB_INSTRUCTION',
          proposal_id: 'proposal-a',
          ...
          native_completed_at: '2026-08-23T10:20:00.000Z',
          outbox_event_id: 'mc2:ROLE_GUIDE:event',
        },
      ],
      error: null,
    });
    ...
    await expect(
      freshRepository.lookup(binding.bindingId, jobCommand.commandId)
  ...
  it('maps concrete native observation to uncertain takeover or signed completion proof', async () => {
    const rpc = vi
      .fn()
      .mockResolvedValueOnce({
        data: [{ outcome: 'running', native_completed_at: null, outbox_event_id: null }],
        error: null,
      })
      .mockResolvedValueOnce({
        data: [
          {
            outcome: 'completed',
            native_completed_at: '2026-08-23T10:30:00.000Z',
            outbox_event_id: 'mc2:COURSE:event',
          },
        ],
        error: null,
      });
  ...
  it('reserves one object, schedules once, exact-replays stably, and conflicts on a changed hash', async () => {
    ...
    await expect(
      dispatchHelixaGenerationCommand({ ...input, command: changed })
    ).resolves.toMatchObject({
      state: 'conflict',
      error: { code: 'megacampus_generation_command_conflict' },
    });
    expect(nativePort.schedule).toHaveBeenCalledTimes(1);
  });
  ...
  it('builds the native ROLE_GUIDE queue input without dropping pane fields', async () => {
    ...
    expect(scheduleRoleGuide).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('uses one atomic native Course transaction and maps a stale current ROLE_GUIDE before mutation', async () => {
    const scheduleCourseFromRoleGuide = vi.fn(async () => {
      throw new Error('ROLE_GUIDE_SOURCE_STALE');
    });
    const port = createHelixaGenerationNativePort({
    ...
    await expect(
      port.schedule({
        ...
        command: parseHelixaGenerationCommand(courseCommand),
        objectKind: 'COURSE',
        objectId: '44444444-4444-4444-8444-444444444444',
        servicePrincipalUserId: binding.servicePrincipalUserId,
        leaseToken: '66666666-6666-4666-8666-666666666666',
    ...
    expect(scheduleCourseFromRoleGuide).toHaveBeenCalledWith(
      expect.objectContaining({
        courseId: '44444444-4444-4444-8444-444444444444',
        sourceJobInstruction: courseCommand.sourceJobInstruction,
        originBindingId: binding.bindingId,
  ...
  it('observes a fenced scheduled terminal failure without replaying native mutation', async () => {
    ...
    expect(repository.failObserved).toHaveBeenCalledWith(
  ...
  it('returns native success to scheduled while waiting for signed completion proof', async () => {
    ...
    const result = await observeHelixaScheduledGenerationCommand({
      ...
      nativePort: {
        schedule: vi.fn(),
        observe: vi.fn(async () => 'succeeded_awaiting_signed_import' as const),
      },
    });
  ...
  it('runs fenced native failure observation through the server lookup entrypoint', async () => {
    ...
    repository.lookup
      .mockResolvedValueOnce({
        ...(await repository.claimScheduled()),
        status: 'scheduled',
        leaseToken: null,
      })
      .mockResolvedValueOnce({
    ...
    expect(result).toMatchObject({
      ...
      error: { code: 'megacampus_generation_native_failed' },
    });
  ...
  it.each(['running', 'succeeded_awaiting_signed_import'] as const)(
    ...
    async observed => {
      ...
      await expect(dispatchHelixaGenerationCommand(input)).resolves.toMatchObject({
        state: 'accepted',
      });
      expect(schedule).toHaveBeenCalledOnce();
      expect(nativePort.observe).toHaveBeenCalledOnce();
    ...
    await expect(
      observeHelixaScheduledGenerationCommand({
        ...
        nativePort: {
          schedule,
          observe: vi.fn(async () => {
            throw new Error('observer response lost');
          }),
        },
      })
  ...
  it('preserves an ambiguous lost response for takeover reconciliation without a second native mutation', async () => {
    const row = {
      ...
      commandId: jobCommand.commandId,
      commandHash: generationCommandHash(parseHelixaGenerationCommand(jobCommand)),
      operation: 'CREATE_JOB_INSTRUCTION' as const,
      payloadHash: jobCommand.payloadHash,
      proposalId: 'proposal-a',
      approvedRevision: 3,
      objectKind: 'ROLE_GUIDE' as const,
      objectId: '33333333-3333-4333-8333-333333333333',
      status: 'reserved' as const,
      acceptedAt: null,
      updatedAt: '2026-08-23T10:16:00.000Z',
      claimGeneration: 1,
      leaseToken: '66666666-6666-4666-8666-666666666666',
    };
    ...
    const schedule = vi.fn().mockImplementationOnce(async () => {
    ...
    const nativePort = {
      ...
      reconcile: vi.fn(async () => ({
        kind: 'completed' as const,
        nativeCompletedAt: '2026-08-23T10:20:00.000Z',
        outboxEventId: 'mc2:ROLE_GUIDE:event',
      })),
    };
    const dispatch = () =>
