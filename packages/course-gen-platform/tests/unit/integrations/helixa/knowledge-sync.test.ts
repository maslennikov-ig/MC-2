...
function snapshot(overrides: Partial<KnowledgeExportSnapshot> = {}): KnowledgeExportSnapshot {
  return {
    kind: 'COURSE', id: courseId, organizationId, completedAt,
    title: 'Safe operations', language: 'ru',
    url: 'https://ai.megacampus.ru/courses/course-1',
    summaryMarkdown: '# Safe operations',
    structure: { z: 1, a: { y: 2, x: 1 } }, blocks: [],
...
describe('MegaCampus knowledge package', () => {
  it('freezes canonical bytes, payload hash, source bytes, and stable identities', async () => {
    const input = snapshot({ sources: [{
      id: 'file-1', sourceType: 'file_catalog', organizationId, objectKind: 'COURSE',
      objectId: courseId, approved: true, version: 'v1', fileName: 'policy.txt',
      mediaType: 'text/plain', readOriginalBytes: async () => Buffer.from('policy bytes'),
    ...
    expect(first.hashes.payloadHash).toBe(createHash('sha256').update(canonical(projected), 'utf8').digest('hex'));
  });
  ...
  it('builds ROLE_GUIDE and preserves accepted Docling JSON', async () => {
    ...
    const result = await buildKnowledgeSyncPackage(snapshot({
      kind: 'ROLE_GUIDE', id: 'playbook-1', title: 'Sales Lead', summaryMarkdown: '# Sales Lead',
      structure: { roleProfileSpec: { level: 'lead' } }, blocks: [{ key: 'responsibilities', markdown: 'Own sales.' }], lessons: [],
      sources: [{ id: 'source-1', sourceType: 'career_playbook_source', organizationId,
        objectKind: 'ROLE_GUIDE', objectId: 'playbook-1', approved: true, version: 'v2',
        fileName: 'source.pdf', mediaType: 'application/pdf',
        readOriginalBytes: async () => Buffer.from('%PDF fixture'), acceptedDoclingJson: docling }],
  ...
  it('fails closed for unapproved, cross-tenant, or cross-object sources', async () => {
    for (const source of [{ approved: false }, { organizationId: 'other-org' }, { objectId: 'other-course' }]) {
      await expect(buildKnowledgeSyncPackage(snapshot({ sources: [{
        id: 'file-1', sourceType: 'file_catalog', organizationId, objectKind: 'COURSE', objectId: courseId,
        approved: true, version: 'v1', fileName: 'policy.txt', mediaType: 'text/plain',
        readOriginalBytes: async () => Buffer.from('private'), ...source,
      }] }), { environment: 'test', externalProjectId: null })).rejects.toThrow(/provenance/i);
    }
  });
  ...
  it('routes every frozen family and counts unsupported', async () => {
    const routes = new Map([
      ...
      ['text/html', 'docling'],
      ['image/png', 'docling'],
      ['text/plain', 'local_text'], ['text/markdown', 'local_text'], ['application/json', 'local_text'],
      ['application/rss+xml', 'content_rss'], ['application/atom+xml', 'content_rss'],
      ['audio/mpeg', 'meetings_media'], ['video/mp4', 'meetings_media'],
      ['application/x-custom', 'unsupported'],
    ]);
    const mediaTypes = [...routes.keys()];
    const result = await buildKnowledgeSyncPackage(snapshot({ sources: mediaTypes.map((mediaType, index) => ({
      id: `file-${index}`, sourceType: 'file_catalog' as const, organizationId, objectKind: 'COURSE' as const,
  ...
  it('fails the package when approved source bytes cannot be read and safely keeps an original-only source', async () => {
    ...
    const originalOnly = await buildKnowledgeSyncPackage(snapshot({ sources: [{
      ...
      approved: true, version: 'v1', fileName: 'raw.pdf', mediaType: 'application/pdf',
      readOriginalBytes: async () => Buffer.from('raw'),
    }] }), { environment: 'test', externalProjectId: null });
  ...
  it('maps actual completed Stage 6 and Career Playbook records fail-closed', async () => {
    const course = await mapCompletedCourse({
      course: { id: courseId, organization_id: organizationId, generation_status: 'completed', generation_completed_at: completedAt, title: 'Course', language: 'ru', course_structure: { sections: [] } },
      lessonContents: [{ lesson_id: 'lesson-1', status: 'completed', content: { markdown: '# Lesson' }, metadata: { lessonLabel: '1.1' } }],
      files: [], readBytes: vi.fn(),
    });
    expect(course).toMatchObject({ kind: 'COURSE', structure: { sections: [] }, lessons: [{ lesson_id: 'lesson-1' }] });

    const guide = await mapCompletedRoleGuide({
      playbook: { id: 'role-1', organization_id: organizationId, status: 'completed', completed_at: completedAt,
        position_title: 'Sales Lead', language: 'en', final_markdown: '# Sales Lead', role_profile_spec: { level: 'lead' }, generated_blocks: { mission: { markdown: 'Grow.' } } },
...
describe('delivery and durable intent', () => {
  ...
  it('signs exact stored bytes and sends injected routing headers', async () => {
    ...
    await deliverClaimedKnowledgeSync({ id: 'outbox-1', eventId: 'event-1', rawBody }, {
      endpoint: 'https://helixa.test/api/integrations/megacampus/knowledge-sync', hmacKey: 'test-only-key',
      externalSystemId: 'system-1', request,
    });
    expect(request).toHaveBeenCalledWith(expect.objectContaining({ body: rawBody, headers: {
      'Content-Type': 'application/json', 'X-Helixa-External-System-Id': 'system-1',
  ...
  it('classifies retryable and terminal responses exactly', () => {
    ...
    for (const status of [400, 401, 403, 404, 409, 422]) expect(classifyDeliveryFailure({ kind: 'http', status, message: 'failed' })).toBe('terminal');
  });
  ...
  it('migration atomically captures both completion transitions without changing job_outbox', async () => {
    const sql = await readFile(new URL('../../../../supabase/migrations/20260822235900_helixa_knowledge_sync_outbox.sql', import.meta.url), 'utf8');
    expect(sql).toContain('CREATE TABLE helixa_knowledge_sync_outbox');
    expect(sql).toContain('AFTER INSERT OR UPDATE OF generation_status, generation_completed_at ON courses');
  ...
  it('reconciler defaults to dry-run and applies deterministic missing intents explicitly', async () => {
    ...
    expect(await reconcileCompletedKnowledgeObjects({ listCompleted, insertMissing })).toMatchObject({ dryRun: true, discovered: 2, inserted: 0 });
    expect(insertMissing).not.toHaveBeenCalled();
    expect(await reconcileCompletedKnowledgeObjects({ listCompleted, insertMissing }, { apply: true })).toMatchObject({ dryRun: false, discovered: 2, inserted: 2 });
    expect(insertMissing).toHaveBeenCalledWith(expect.arrayContaining([
