...
describe.runIf(RUN_REAL_CONTROLLER)(
  'Q12 live cutover controller (Task-9) — R3 resource-manifest 2-step binding',
  () => {
    ...
    it('fails closed when a production request targets a non-production run root', async () => {
      await expect(
        materializeLiveController({ runRoot: root(), production: true })
      ).rejects.toThrow(/production run root mismatch/u);
    });
  }
);
...
describe.runIf(RUN_REAL_CONTROLLER)(
  'Q12 live cutover controller (Task-9) — R4 Sub-round A: injectable ordinary execution seam',
  () => {
...
describe.runIf(RUN_REAL_CONTROLLER)(
  'Q12 live cutover controller (Task-9) — R4 Sub-round B: real deployed wrapper barrier claims',
  () => {
    it('drives every in-process barrier claim through the REAL deployed q12-capability-run.sh wrapper without perturbing the journal twin', async () => {
      ...
      const audit = JSON.parse(
        readFileSync(join(liveRoot, 'executor-audit.json'), 'utf8')
...
describe.runIf(RUN_REAL_CONTROLLER)(
  'Q12 live cutover controller (Task-9) — R5 Sub-round A: forward final-writer manifest (FWM) parity',
  () => {
    it('journals groups 14-16 (FWM, deploy.commit, barrier.activate) as a full 76-row byte/order twin of the composer with root-independent FWM-content byte parity', async () => {
      ...
      expect(
        live.journalEntries.slice(68, 76).map(r => [r.phase, r.outcome, r.command_id])
      ).toEqual([
...
describe.runIf(RUN_REAL_CONTROLLER)(
  'Q12 live cutover controller (Task-9) — R5 Sub-round C: cutover-window marker',
  () => {
...
describe.runIf(RUN_REAL_CONTROLLER)(
  'Q12 live cutover controller (Task-9) — R8-I-A: journaled post-activate barrier.cleanup segment',
  () => {
...
describe.runIf(RUN_REAL_CONTROLLER)(
  'Q12 live cutover controller (Task-9) — R5 Sub-round C close: marker present before group-3 writers.quiesce',
  () => {
...
describe.runIf(RUN_REAL_CONTROLLER)(
  'Q12 live cutover controller (Task-9) — R5 Sub-round D: run_recover',
  () => {
    ...
    it('recovers from the crash-after-FWM restart (writers.resume.forward/accepted) to the full 76-row twin + post-activate', async () => {
      ...
      const fwmEnd =
        witnessIndex(composer.journalEntries, 'writers.resume.forward', 'accepted') + 1;
...
describe.runIf(RUN_REAL_CONTROLLER)(
  'Q12 live cutover controller (Task-9) — R8-I-B: generalized Option A recover head-dispatch',
  () => {
...
describe.runIf(RUN_REAL_CONTROLLER)(
  'Q12 live cutover controller (Task-9) — R8-I-C: §6b.6 composed mid-barrier recovery probes',
  () => {
