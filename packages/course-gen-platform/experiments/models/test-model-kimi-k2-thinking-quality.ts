#!/usr/bin/env tsx
/**
 * Quality-Focused Model Test: moonshotai/kimi-k2-thinking
 *
 * Executes 3 runs per scenario (4 scenarios = 12 total API calls)
 * Saves full JSON outputs to /tmp/quality-tests/kimi-k2-thinking/
 *
 * Test Scenarios:
 * 1. metadata-en: "Introduction to Python Programming" (3 runs)
 * 2. metadata-ru: "Машинное обучение для начинающих" (3 runs)
 * 3. lesson-en: "Variables and Data Types in Python" (3 runs)
 * 4. lesson-ru: "Основы нейронных сетей" (3 runs)
 *
 * Methodology: docs/MODEL-QUALITY-TESTING-METHODOLOGY-V2.md
 *
 * Usage:
 *   pnpm tsx experiments/models/test-model-kimi-k2-thinking-quality.ts
 */
 ...
 * QUALITY_TESTS_DIR=specs/008-generation-generation-json/quality-tests pnpm tsx experiments/models/test-model-kimi-k2-thinking-quality.ts
 */
const BASE_QUALITY_DIR = process.env.QUALITY_TESTS_DIR || '.tmp/quality-tests';

class KimiK2ThinkingTester {
  private apiKey: string;
  private modelName = 'moonshotai/kimi-k2-thinking';
  private modelSlug = 'kimi-k2-thinking';
  private outputDir = `${BASE_QUALITY_DIR}/kimi-k2-thinking`;
...
Output the JSON directly (no markdown, no explanations):`;
  ...
  private async runTest(scenario: TestScenario, runNumber: number): Promise<TestRun> {
    ...
    try {
      ...
      log(
        `[${this.modelSlug}] ${scenario.id} run ${runNumber}/${this.runsPerScenario}... ✓ ${duration}ms (${result.tokens.output} tokens)`,
        'success'
      );
      ...
      log(
        `[${this.modelSlug}] ${scenario.id} run ${runNumber}/${this.runsPerScenario}... ✗ ${err.error?.message || 'Unknown error'}`,
        'error'
      );
  ...
  printSummary(): void {
    ...
    console.log(`\nResults:`);
    console.log(`  ${colors.green}✓${colors.reset} Passed: ${passed}/${total}`);
    if (failed > 0) {
      console.log(`  ${colors.red}✗${colors.reset} Failed: ${failed}/${total}`);
    }
    ...
    console.log(`  3. Run quality analysis (manual or automated)`);
  }
}

async function main() {
  try {
    const tester = new KimiK2ThinkingTester();
