#!/usr/bin/env tsx
/**
 * Model Evaluation Test: moonshotai/kimi-k2-0905
 *
 * Executes 4 test cases to get complete token data for comparison with kimi-k2-thinking
 *
 * Test Cases:
 ...
 *
 * Output: /tmp/kimi-k2-0905-complete.log
 *
 * Usage:
 *   pnpm tsx experiments/models/test-model-kimi-k2-0905.ts
 */
...
5. Output ONLY valid JSON, no markdown or explanations`;
  ...
  async testMetadataEnglish(): Promise<void> {
    ...
    try {
      ...
      // Pricing: $0.35 input / $1.4 output per 1M tokens
      const cost = (result.tokens.input * 0.35 + result.tokens.output * 1.4) / 1000000;
  ...
  printSummary(): void {
    section('Test Summary: moonshotai/kimi-k2-0905');
  ...
  async runAll(): Promise<void> {
    section('Model Evaluation: moonshotai/kimi-k2-0905');
    log('Testing all 4 scenarios for complete token data', 'info');
    log('Pricing: $0.35 input / $1.4 output per 1M tokens', 'info');
