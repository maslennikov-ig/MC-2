#!/usr/bin/env tsx
/**
 * Model Evaluation Test: deepseek/deepseek-chat-v3.1
 *
 * Re-testing with precise token measurements (previous data was approximate)
 *
 * Test Cases:
 ...
 *
 * Output: /tmp/deepseek-chat-v31-complete.log
 *
 * Usage:
 *   pnpm tsx experiments/models/test-model-deepseek-chat-v31.ts
 */
...
5. Output ONLY valid JSON, no markdown or explanations`;
  ...
  async testMetadataEnglish(): Promise<void> {
    ...
    try {
      ...
      // Pricing: $0.27 input / $1.1 output per 1M tokens
      const cost = (result.tokens.input * 0.27 + result.tokens.output * 1.1) / 1000000;
  ...
  printSummary(): void {
    section('Test Summary: deepseek/deepseek-chat-v3.1');
    ...
    if (lessonTests.length > 0) {
      ...
      log('NOTE: Previous data showed quality 0.80 and simplified schema (camelCase, 1 lesson)', 'warning');
    }
  }

  async runAll(): Promise<void> {
    section('Model Evaluation: deepseek/deepseek-chat-v3.1');
    log('Re-testing with precise measurements', 'info');
    log('Pricing: $0.27 input / $1.1 output per 1M tokens', 'info');
