#!/usr/bin/env tsx
/**
 * Model Evaluation Test: deepseek/deepseek-v3.2-exp
 *
 * Complete testing - currently no token data available
 *
 * Test Cases:
 ...
 *
 * Output: /tmp/deepseek-v32-exp-complete.log
 *
 * Usage:
 *   pnpm tsx experiments/models/test-model-deepseek-v32-exp.ts
 */
...
5. Output ONLY valid JSON, no markdown or explanations`;
  ...
  async testMetadataEnglish(): Promise<void> {
    ...
    try {
      ...
      // Pricing: $0.07 input / $0.33 output per 1M tokens (CHEAPEST!)
      const cost = (result.tokens.input * 0.07 + result.tokens.output * 0.33) / 1000000;
  ...
  printSummary(): void {
    section('Test Summary: deepseek/deepseek-v3.2-exp');
  ...
  async runAll(): Promise<void> {
    section('Model Evaluation: deepseek/deepseek-v3.2-exp');
    log('Testing all 4 scenarios for cheapest model', 'info');
    log('Pricing: $0.07 input / $0.33 output per 1M tokens', 'info');
