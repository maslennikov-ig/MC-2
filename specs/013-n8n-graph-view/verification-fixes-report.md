...
2.  **Infinite Render Loop (`Maximum update depth exceeded`)**:
    ...
    - **Fix:** Implemented **Deep Comparison** (`JSON.stringify`) inside the `setNodes` and `setEdges` functional updates. State is now only updated if the _content_ actually changes.
