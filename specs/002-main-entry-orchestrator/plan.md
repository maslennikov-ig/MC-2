...
- ✅ Saga pattern implemented for RPC failures (retry 3x with exponential backoff, rollback job on failure)
- ✅ Worker fallback recovery for orphaned jobs (step 1 completion check)
- ✅ Idempotent RPC design allows safe retries
- ✅ Progress persisted at each stage (JSONB updates atomic)
- ✅ System metrics table tracks all failures (job_rollback, orphaned_job_recovery)

### ✅ II. Atomicity & Modularity

**Compliance**: Fully compliant

- ✅ Concurrency tracker: 150 lines (single responsibility)
- ✅ Retry utility: 30 lines (pure function)
- ✅ API endpoint: ~200 lines (orchestration logic only)
- ✅ Worker recovery: ~50 lines (separate concern)
...
- ✅ Data model: `data-model.md` (entities, schema, migrations)
- ✅ API contracts: `contracts/api-endpoint.md`, `contracts/rpc-update-course-progress.md`
- ✅ Quickstart: `quickstart.md` (local dev setup)
- ⏳ Tasks: `tasks.md` (generated via `/speckit.tasks`)

### ✅ IV. Incremental Testing

**Compliance**: Fully compliant

- ✅ Unit tests planned for concurrency tracker, retry util, API validation
- ✅ Integration tests planned for full orchestration flow (endpoint → job → progress update)
- ✅ Contract tests planned for JSON schema validation (200, 401, 403, 404, 429, 500 responses)
- ✅ RPC function tests planned using pgTAP (SQL unit tests)
- ✅ Manual E2E testing via BullBoard dashboard

### ✅ V. Observability & Monitoring

**Compliance**: Fully compliant

- ✅ Pino structured JSON logging (level, timestamp, context)
- ✅ Child loggers with context propagation (requestId, userId, tier, courseId, jobId)
- ✅ System metrics table for critical events (job_rollback, orphaned_job_recovery, concurrency_limit_hit)
- ✅ Log levels: debug (dev only), info, warn, error
- ✅ Correlation IDs via nanoid() for request tracing
...
- ✅ Course ownership verified (RLS + application layer)
- ✅ Per-user concurrency limits enforced (tier-based)
- ✅ Queue priorities based on subscription tier
- ✅ Redis counters enable horizontal scaling
- ✅ BullMQ supports distributed workers

### ⚠️ VII. AI Model Flexibility

**Compliance**: Not applicable (Stage 1 has no AI model calls)

- ℹ️ Stage 1 is pure orchestration (no prompts, no AI calls)
- ℹ️ AI model usage begins in Stage 2+ (document processing, summarization)
- ✅ Architecture supports future flexibility (workers call AI modules)
...
│   ├── api-endpoint.md      #    POST /api/coursegen/generate contract
│   └── rpc-update-course-progress.md  # RPC function contract
...
├── app/api/coursegen/generate/
│   └── route.ts                         # 🆕 NEW: API endpoint
├── lib/
│   └── supabase/
│       └── server.ts                    # Existing: Auth helpers
└── supabase/migrations/
    ├── {timestamp}_create_system_metrics.sql        # 🆕 NEW: system_metrics table
    └── {timestamp}_create_update_course_progress_rpc.sql  # 🆕 NEW: RPC function

packages/course-gen-platform/            # Backend tRPC server
├── src/
│   ├── orchestrator/
│   │   ├── handlers/
│   │   │   ├── base-handler.ts          # ✏️ MODIFIED: Add step 1 recovery
│   │   │   └── document-processing.ts   # ✏️ MODIFIED: Add orphan detection
│   │   ├── queue.ts                     # Existing: BullMQ queue
│   │   └── worker.ts                    # Existing: Job processor
│   ├── shared/
│   │   ├── concurrency/
│   │   │   └── tracker.ts               # 🆕 NEW: Concurrency enforcement
│   │   ├── logger/
│   │   │   └── index.ts                 # ✏️ MODIFIED: Replace with Pino
│   │   ├── utils/
│   │   │   └── retry.ts                 # 🆕 NEW: Retry with backoff
│   │   └── types/
│   │       ├── system-metrics.ts        # 🆕 NEW: Metrics types
│   │       └── concurrency.ts           # 🆕 NEW: Concurrency types
│   └── server/
│       └── index.ts                     # Existing: tRPC server

packages/shared-types/                   # Shared TypeScript types
├── src/
│   ├── bullmq-jobs.ts                   # Existing: Job types (no changes)
│   └── index.ts                         # Existing: Exports

tests/                                   # Integration tests
├── integration/
│   ├── orchestrator/
│   │   └── generate-endpoint.test.ts    # 🆕 NEW: E2E orchestration tests
│   └── fixtures/
│       └── test-courses.ts              # 🆕 NEW: Test data
└── unit/
    ├── concurrency-tracker.test.ts      # 🆕 NEW: Concurrency tests
    └── retry-util.test.ts               # 🆕 NEW: Retry logic tests
```
...
- ✅ **database-architect** - Database migrations, RPC functions, schema design, PostgreSQL expertise
- ✅ **api-builder** - tRPC/REST endpoints, Next.js App Router, authentication, API validation
- ✅ **infrastructure-specialist** - Redis, BullMQ, worker config, queue management, concurrency
- ✅ **fullstack-nextjs-specialist** - Complex Next.js frontend + backend integration, React patterns
- ✅ **integration-tester** - E2E workflow testing, contract validation, acceptance tests
- ✅ **code-reviewer** - Quality validation, constitution compliance checks, best practices
...
- **Database Migrations (BLOCKING)**:
  - T003 → T004 → T005 (create system_metrics → create RPC → apply migrations)
...
- **API Endpoint Incremental Build**:
  ...
  - **Reason**: Building single file `route.ts` incrementally (skeleton → auth → ownership → concurrency → branching → job → RPC → rollback → success)
...
- **Individual Blockers**:
  - T005 (Apply migrations) → BLOCKS → T006-T010 (can't create types for table that doesn't exist)
  - T008 (Concurrency types) → BLOCKS → T010 (concurrency tracker needs type definitions)
  - T010 (Concurrency tracker) → BLOCKS → Phase 3 (API endpoint needs concurrency.checkAndReserve())
...
1. ✅ **Pino Logger Implementation** → Drop-in replacement with child logger pattern, 10x faster than Winston
2. ✅ **System Metrics Table Design** → Event-based table with JSONB metadata, indexed by event_type and severity
3. ✅ **RPC `update_course_progress` Implementation** → Idempotent PostgreSQL function with atomic JSONB updates
4. ✅ **Concurrency Limit Enforcement** → Redis counters with Lua scripts for atomic check-and-reserve
5. ✅ **Dynamic Global Concurrency (Stage 8 Preview)** → Hardcoded limit 3 in Stage 1, dynamic 3-10 in Stage 8
6. ✅ **Saga Pattern Implementation** → Try-catch with explicit rollback in orchestrator, fallback in worker
...
- [ ] Tasks generated via `/speckit.tasks` ← **Next step**
...
- ✅ RPC Function - Step validation, percentage calculation, JSONB updates
...
4. ✅ Worker orphan recovery: Skip step 1 RPC, verify worker completes it
5. ✅ Priority queueing: Submit jobs from different tiers, verify processing order
...
- ✅ RPC return value matches `GenerationProgress` type
...
4. ✅ Verify Russian step names in UI
5. ✅ Trigger concurrency limit, verify 429 error
6. ✅ Manually delete Redis counter mid-job, verify worker recovery
...
- [ ] Saga pattern tested: RPC failure → job rollback
- [ ] Worker recovery tested: missing step 1 → auto-complete
...
- ❌ Database migration: `system_metrics` table
- ❌ Database migration: `update_course_progress` RPC function
- ❌ Pino logger installed and configured
- ❌ Concurrency tracker module
- ❌ API endpoint `/api/coursegen/generate`
- ❌ JWT Bearer token validation middleware
- ❌ Worker orphan recovery logic
- ❌ Retry utility with exponential backoff
- ❌ System metrics logging helper
...
4. ✅ Concurrency enforcement approach
5. ✅ Dynamic concurrency (deferred to Stage 8)
