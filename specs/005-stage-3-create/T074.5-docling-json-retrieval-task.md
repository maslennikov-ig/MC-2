...
- ✅ No MCP server fork required (uses original image)
- ✅ Full metadata: texts[], pictures[], tables[], pages{}
- ✅ Preserves caching via document_key
- ✅ Easy upgrades (no custom maintenance)
...
1. **`extractImageMetadata()`** → Returns `[]` (no images found)
...
2. **`extractDocumentStructure()`** → Cannot find real page numbers
...
- ✅ `convert_document_into_docling_document` → Returns only `document_key` (string)
- ✅ `export_docling_document_to_markdown` → Returns markdown (string)
- ❌ `export_docling_document_to_json` → **Does NOT exist**
...
- [ ] Integration test: Convert 5-page PDF with 3 images → verify all fields populated
...
### Option A: Add `export_docling_document_to_json` Tool to Docling MCP (Recommended ⭐)
...
- ⚠️ Requires upstream PR to docling-mcp project
- ⚠️ Wait time for merge/release (1-2 weeks)
- ⚠️ Need to maintain fork if PR not accepted
...
- ✅ Full control over Docling
- ✅ No upstream dependency
- ✅ Immediate implementation

**Cons**:

- ⚠️ Loses MCP caching benefits
- ⚠️ Additional process overhead
- ⚠️ Complex Python environment management
- ⚠️ Harder to debug
...
- ❌ No OCR text from images
- ❌ No bbox coordinates
- ❌ No base64 image data
- ❌ Approximate metadata only
- ❌ Cannot extract font information
...
- If docling-mcp is actively maintained (commits <30 days) → **Option A**
- If upstream PRs are slow (>2 weeks merge time) → **Option B**
- If need immediate solution → **Option C** (temporary), then **Option A** later
...
- Current: 2 calls (convert → export markdown)
- Proposed: 2 calls (convert → export JSON)
